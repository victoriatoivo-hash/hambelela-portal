...
try {
 ...
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $hash=hash_file('sha256',$tmp); $find=db()->prepare('SELECT id FROM accounts_paye_statements WHERE sha256=?'); $find->execute([$hash]);
  if($find->fetchColumn()){paye_reclassify_existing_rows();paye_reply(['ok'=>true,'message'=>'This statement was already uploaded. Existing rows were reprocessed against the latest return and payment rules.','data'=>paye_payload($month)]);}
  $dir=BASE_PATH.'/uploads/paye-statements'; if(!is_dir($dir)&&!mkdir($dir,0750,true)&&!is_dir($dir))throw new RuntimeException('Protected PAYE statement storage is unavailable.'); $stored=bin2hex(random_bytes(24)).'.'.$allowed[$mime]; $path=$dir.'/'.$stored; if(!move_uploaded_file($tmp,$path))throw new RuntimeException('The PAYE statement could not be stored.');
  try{
   $engine='csv';
   ...
   db()->prepare('INSERT INTO accounts_paye_statements(original_filename,stored_filename,mime_type,file_size,sha256,statement_period,status,parse_message,rows_detected,uploaded_by,uploaded_by_name)VALUES(?,?,?,?,?,?,?,?,?,?,?)')->execute([mb_substr(basename((string)$file['name']),0,255),$stored,$mime,$size,$hash,$_POST['statement_period']??null,'processed',count($rows).' PAYE transaction rows extracted using '.$engine.'. Penalties and interest are excluded from principal payable.',count($rows),(int)$user['id'],(string)$user['name']]);
   $statementId=(int)db()->lastInsertId(); $insert=db()->prepare('INSERT IGNORE INTO accounts_paye_rows(statement_id,source_row_number,tax_year,tax_period,accounting_period,due_date,action_date,reference,transaction_type,classification,amount,source_hash,source_json)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)'); $saved=0;
   ...
   if(!$saved)throw new RuntimeException('No new valid PAYE rows were available after period and duplicate validation.'); paye_audit($statementId,'statement_processed',['rows'=>$saved,'extractor'=>$engine,'penalty_interest_excluded'=>true]); db()->commit(); paye_reply(['ok'=>true,'message'=>$saved.' PAYE rows processed and matched by return period.','data'=>paye_payload($month)]);
