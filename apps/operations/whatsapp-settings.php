...
$pageTitle = 'WhatsApp Settings | ' . APP_NAME;
...
$logs = $ready && ops_table_exists('ops_whatsapp_webhook_logs') ? ops_rows(
    "SELECT event_type, phone_number_id, wa_message_id, processed_status, error_message, received_at
...
<main class="workspace module whatsapp-module">
    <section class="module-header cost-system-header whatsapp-hero">
        <div>
            ...
            <p>Connect Meta Cloud API, configure webhook verification, test connection health and monitor webhook events.</p>
        </div>
        <div class="actions">
            <a class="button" href="whatsapp.php"><i data-lucide="arrow-left"></i> WhatsApp KPI</a>
        </div>
    </section>
    ...
    <section class="panel whatsapp-api-strip">
        ...
        <div><span>Webhook status</span><strong><?= htmlspecialchars($settings['last_webhook_received_at'] === 'Never' ? 'Waiting' : 'Receiving', ENT_QUOTES, 'UTF-8') ?></strong></div>
        <div><span>Last webhook</span><strong><?= htmlspecialchars($settings['last_webhook_received_at'], ENT_QUOTES, 'UTF-8') ?></strong></div>
        <div><span>Connection</span><strong><?= htmlspecialchars($settings['last_connection_test_status'], ENT_QUOTES, 'UTF-8') ?></strong></div>
    </section>

    <section class="whatsapp-workspace-grid">
        <article class="panel">
            <div class="section-row"><h2>Meta Cloud API settings</h2><span class="status">Owner/Admin only</span></div>
            <form class="form-grid" method="post">
                ...
                <label>Permanent Access Token<input name="access_token" type="password" value="__KEEP__" autocomplete="new-password"><small>Stored token: <?= htmlspecialchars(wa_mask_secret($settings['access_token']), ENT_QUOTES, 'UTF-8') ?></small></label>
        ...
        <article class="panel">
            ...
            <p>Use this after saving the Phone Number ID and permanent access token. It calls Meta Graph API and records the result.</p>
            ...
            <div class="whatsapp-settings-help">
                ...
                <p>Subscribe to WhatsApp message and status webhook events.</p>
            </div>
        </article>
    </section>
    ...
    <section class="panel">
        ...
        <div class="table-scroll">
            <table class="data-table ops-table">
                <thead><tr><th>Received</th><th>Type</th><th>Phone ID</th><th>Message ID</th><th>Status</th><th>Error</th></tr></thead>
                <tbody>
                ...
                <?php endforeach; ?>
                <?php if (!$logs): ?><tr><td colspan="6">No webhook events received yet.</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
    </section>
