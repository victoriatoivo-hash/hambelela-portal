...
function kpi_packer_epi_orders_evidence(int $employeeId,string $fromSql,string $toSql,array $settings,array $holidays):?array
{
    ...
    if(!$events&&ops_table_exists('kpi_activity_events')){$hasWeight=ops_column_exists('ops_orders','total_weight_kg');$weightSelect=$hasWeight?'o.total_weight_kg':'NULL total_weight_kg';$createdExpr=ops_order_display_datetime_expr('o');$wooJoin=ops_column_exists('ops_orders','woo_order_id')?' OR o.woo_order_id=ev.record_id':'';$transitions=ops_rows("SELECT ev.id evidence_id,o.id order_id,ev.occurred_at,ev.previous_status status_before,ev.new_status status_after,ev.employee_name packed_by,o.order_number,o.customer_name,o.status,o.assigned_at,{$createdExpr} portal_imported_at,COALESCE(NULLIF(o.fulfilment_mode,''),o.order_type) fulfilment_mode,{$weightSelect},COALESCE(items.item_quantity,0) item_quantity FROM kpi_activity_events ev JOIN ops_orders o ON (o.id=ev.record_id{$wooJoin} OR (ev.related_reference IS NOT NULL AND ev.related_reference<>'' AND o.order_number=ev.related_reference)) LEFT JOIN (SELECT order_id,SUM(quantity) item_quantity FROM ops_order_items GROUP BY order_id) items ON items.order_id=o.id WHERE ev.employee_id=? AND ev.portal_section='orders' AND ev.record_type='order' AND ev.action_key='status_changed' AND LOWER(REPLACE(ev.new_status,'_',''))='inprogress' AND ev.occurred_at BETWEEN ? AND ? ORDER BY ev.occurred_at DESC,ev.id DESC",[$employeeId,$fromSql,$toSql]);$seenTransition=[];foreach($transitions as$transition){$recordId=(int)$transition['order_id'];if(isset($seenTransition[$recordId]))continue;$seenTransition[$recordId]=true;$start=!empty($transition['portal_imported_at'])?new DateTimeImmutable((string)$transition['portal_imported_at'],new DateTimeZone('Africa/Windhoek')):null;$done=new DateTimeImmutable((string)$transition['occurred_at'],new DateTimeZone('Africa/Windhoek'));$minutes=$start&&$done>=$start?kpi_business_minutes($start,$done,$holidays):null;$events[]=$transition+['reference_number'=>$transition['order_number'],'working_minutes'=>$minutes,'metadata_json'=>json_encode(['order_type'=>$transition['fulfilment_mode'],'created_at'=>$transition['portal_imported_at'],'evidence_source'=>'kpi_activity_events'])];}}
