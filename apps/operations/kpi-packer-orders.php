...
function kpi_packer_epi_orders_evidence(int $employeeId,string $fromSql,string $toSql,array $settings,array $holidays):?array
{
    ...
    if(!$events){$transitions=[];foreach(kpi_unified_events($fromSql,$toSql,$employeeId,'orders','order')as$event){$new=kpi_front_order_normalize((string)($event['new_status']??''));$action=kpi_front_order_normalize((string)($event['action']??''));if($new!=='inprogress'&&$action!=='inprogress')continue;$recordId=(int)($event['record_id']??0);if(!$recordId||isset($transitions[$recordId]))continue;$transitions[$recordId]=$event;}if(!$transitions)return null;$ids=array_keys($transitions);$placeholders=implode(',',array_fill(0,count($ids),'?'));$hasWeight=ops_column_exists('ops_orders','total_weight_kg');$weightSelect=$hasWeight?'o.total_weight_kg':'NULL total_weight_kg';$createdExpr=ops_order_display_datetime_expr('o');$orders=ops_rows("SELECT o.id order_id,o.order_number,o.customer_name,o.status,o.assigned_at,{$createdExpr} portal_imported_at,COALESCE(NULLIF(o.fulfilment_mode,''),o.order_type) fulfilment_mode,{$weightSelect},COALESCE(items.item_quantity,0) item_quantity FROM ops_orders o LEFT JOIN (SELECT order_id,SUM(quantity) item_quantity FROM ops_order_items GROUP BY order_id) items ON items.order_id=o.id WHERE o.id IN ({$placeholders})",$ids);$byId=[];foreach($orders as$order)$byId[(int)$order['order_id']]=$order;$events=[];foreach($transitions as$recordId=>$transition){$order=$byId[$recordId]??null;if(!$order)continue;$start=!empty($order['portal_imported_at'])?new DateTimeImmutable((string)$order['portal_imported_at'],new DateTimeZone('Africa/Windhoek')):null;$done=new DateTimeImmutable((string)$transition['occurred_at'],new DateTimeZone('Africa/Windhoek'));$minutes=$start&&$done>=$start?kpi_business_minutes($start,$done,$holidays):null;$events[]=$order+['evidence_id'=>$transition['event_id']??$transition['source_event_id']??0,'reference_number'=>$order['order_number'],'occurred_at'=>$transition['occurred_at'],'working_minutes'=>$minutes,'metadata_json'=>json_encode(['order_type'=>$order['fulfilment_mode'],'created_at'=>$order['portal_imported_at'],'evidence_source'=>$transition['source_log']??'unified_activity']),'status_before'=>$transition['previous_status']??'new_order','status_after'=>'in_progress','packed_by'=>$transition['actor_name']??'Employee activity'];}}
