...
function kpi_packer_epi_orders_evidence(int $employeeId,string $fromSql,string $toSql,array $settings,array $holidays):?array
{
    ...
    if(!$events){
        ...
        foreach(kpi_unified_events($fromSql,$toSql,$employeeId)as$event){
            $status=strtolower(str_replace(['_',' '],'',(string)($event['new_status']??'')));
            if((string)($event['section']??'')!=='orders'||(string)($event['record_type']??'')!=='order'||(string)($event['action']??'')!=='status_changed'||$status!=='inprogress')continue;
        ...
        if($normalized){
            ...
            $orders=$conditions?ops_rows("SELECT o.id order_id,o.woo_order_id,o.order_number,o.customer_name,o.status,o.assigned_at,{$createdExpr} portal_imported_at,COALESCE(NULLIF(o.fulfilment_mode,''),o.order_type) fulfilment_mode,0 item_quantity,NULL total_weight_kg FROM ops_orders o WHERE ".implode(' OR ',$conditions),$params):[];
