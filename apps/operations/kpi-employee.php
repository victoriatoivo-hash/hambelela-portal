...
?>
<main class="workspace module kpi-health-page kpi-employee-page" data-employee-id="<?= (int) $employee['id'] ?>" data-initial-anchor="<?= htmlspecialchars($initialAnchor, ENT_QUOTES, 'UTF-8') ?>" data-can-review="<?= $canReview?'1':'0' ?>" data-presence-csrf="<?= htmlspecialchars((string)$_SESSION['kpi_presence_csrf_token'],ENT_QUOTES,'UTF-8') ?>">
  <section class="module-header kpi-employee-header">
    ...
    <div class="kpi-employee-header-actions"><button type="button" class="btn-secondary" data-kpi-refresh>Refresh</button><button type="button" class="btn-secondary" data-kpi-print>Print / Export</button><?php if($canReview): ?><a class="btn-secondary" href="reports.php?tab=employees">Back to employees</a><?php endif; ?></div>
  </section>
