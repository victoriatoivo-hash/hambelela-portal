...
$sections = ['overview'=>'Overview','attendance'=>'Attendance','orders'=>'Orders','packing'=>'Packing','tasks'=>'Tasks','waybills'=>'Waybills','quality'=>'Quality & Errors','trends'=>'Trends','activity'=>'Activity Feed'];
...
<main class="workspace module kpi-health-page kpi-employee-page" data-employee-id="<?= (int) $employee['id'] ?>" data-kpi-section="<?= htmlspecialchars($section,ENT_QUOTES,'UTF-8') ?>">
  <section class="module-header kpi-employee-header"><div class="kpi-employee-identity"><span class="kpi-avatar" aria-hidden="true"><?= htmlspecialchars(strtoupper(substr((string)$employee['full_name'],0,1)),ENT_QUOTES,'UTF-8') ?></span><div><p class="eyebrow">Employee performance</p><h1><?= htmlspecialchars((string)$employee['full_name'],ENT_QUOTES,'UTF-8') ?></h1><p><?= htmlspecialchars((string)$employee['role_name'],ENT_QUOTES,'UTF-8') ?> · <?= htmlspecialchars(ucfirst((string)$employee['status']),ENT_QUOTES,'UTF-8') ?> <span data-kpi-online-state>· Checking presence…</span></p></div></div><a class="btn-secondary" href="reports.php?tab=employees">Back to employees</a></section>
