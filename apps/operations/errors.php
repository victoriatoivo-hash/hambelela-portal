...
function error_parse_occurred_at(string $value): string
{
    ...
    $local = DateTimeImmutable::createFromFormat('!Y-m-d\TH:i', trim($value), $timezone);
    $errors = DateTimeImmutable::getLastErrors();
    if (!$local || (is_array($errors) && (($errors['warning_count'] ?? 0) || ($errors['error_count'] ?? 0)))) throw new RuntimeException('Enter a valid date and time when the error occurred.');
...
function error_bootstrap_schema(): void
{
    ...
    error_try_sql("ALTER TABLE ops_error_field_audit ADD COLUMN IF NOT EXISTS changed_by_user_id INT NULL AFTER changed_by_employee_id");
    error_try_sql("ALTER TABLE ops_error_field_audit ADD COLUMN IF NOT EXISTS changed_by_name VARCHAR(190) NULL AFTER changed_by_user_id");
...
const updateLimitedSaveState=()=>{if(!limitedForm)return;const save=limitedForm.querySelector('[data-error-log-save]');if(!save||limitedForm.dataset.saving==='1')return;const changed=limitedForm.elements.occurred_on.value!==limitedForm.dataset.initialDate||normaliseLimitedImpact(limitedForm.elements.financial_impact.value)!==normaliseLimitedImpact(limitedForm.dataset.initialFinancialImpact);save.disabled=!changed};
errorTaskDetails?.addEventListener('click',(event)=>{const open=event.target.closest('[data-limited-error-edit]');if(open&&limitedDialog){limitedForm.reset();limitedForm.elements.error_id.value=open.dataset.errorId||'';limitedForm.elements.occurred_on.value=open.dataset.occurredOn||'';limitedForm.elements.financial_impact.value=open.dataset.financialImpact||'';limitedForm.dataset.initialDate=open.dataset.occurredOn||'';limitedForm.dataset.initialFinancialImpact=open.dataset.financialImpact||'';for(const key of ['id','employee','title','logged','status']){const node=limitedForm.querySelector(`[data-limited-context="${key}"]`);if(node)node.textContent=key==='id'?`ERROR-${open.dataset.errorId}`:(open.dataset[key]||'—')}const source=limitedForm.querySelector('[data-limited-source]');if(source)source.textContent=open.dataset.occurredOn?'Confirmed or migrated occurrence date':'Estimated from Date Logged';limitedForm.querySelector('[data-limited-feedback]').textContent='';const save=limitedForm.querySelector('[data-error-log-save]');save.textContent='Save changes';save.classList.remove('is-saving','is-saved','has-error');updateLimitedSaveState();limitedDialog.showModal()}if(event.target.closest('[data-error-limited-close]'))limitedDialog?.close()});
limitedForm?.addEventListener('input',updateLimitedSaveState);
const saveLimitedErrorFields=async()=>{if(!limitedForm||limitedForm.dataset.saving==='1')return;const submit=limitedForm.querySelector('[data-error-log-save]'),feedback=limitedForm.querySelector('[data-limited-feedback]'),dateInput=limitedForm.querySelector('[data-error-log-date]'),impactInput=limitedForm.querySelector('[data-error-log-financial-impact]');const date=dateInput.value.trim(),impact=impactInput.value.trim();if(!date){feedback.textContent='Please select a valid error date.';dateInput.focus();return}const normalised=normaliseLimitedImpact(impact);if(normalised!==''&&!/^(?:0|[1-9]\d{0,9})(?:\.\d{1,2})?$/.test(normalised)){feedback.textContent='Financial impact must be zero or a valid positive amount.';impactInput.focus();return}limitedForm.dataset.saving='1';submit.disabled=true;dateInput.disabled=true;impactInput.disabled=true;submit.classList.remove('is-saved','has-error');submit.classList.add('is-saving');submit.textContent='Saving…';feedback.textContent='Saving changes…';try{const payload={csrf:limitedForm.elements.csrf.value,error_id:Number(limitedForm.elements.error_id.value),occurred_on:date,financial_impact:impact===''?null:impact};const response=await fetch(limitedForm.dataset.endpoint,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-Requested-With':'XMLHttpRequest'},body:JSON.stringify(payload)});const result=await response.json().catch(()=>null);if(!response.ok||!result?.success)throw new Error(result?.error||'The changes could not be saved. Please try again.');const id=String(result.error_id),dateCell=document.querySelector(`[data-error-occurred-cell="${id}"]`),financeCell=document.querySelector(`[data-error-finance-cell="${id}"]`),financeDetail=document.querySelector(`[data-error-finance-detail="${id}"]`),editButton=document.querySelector(`[data-limited-error-edit][data-error-id="${id}"]`);if(dateCell)dateCell.textContent=result.error_date_display;if(financeCell)financeCell.textContent=result.financial_impact_display;if(financeDetail)financeDetail.textContent=result.financial_impact_display;if(editButton){editButton.dataset.occurredOn=result.error_date;editButton.dataset.financialImpact=result.financial_impact??''}dateInput.value=result.error_date;impactInput.value=result.financial_impact??'';limitedForm.dataset.initialDate=result.error_date;limitedForm.dataset.initialFinancialImpact=result.financial_impact??'';submit.classList.remove('is-saving','has-error');submit.classList.add('is-saved');submit.textContent='Saved';feedback.textContent=result.changed?'Error Log changes saved.':'No changes were needed.';setTimeout(()=>{submit.classList.remove('is-saved');submit.textContent='Save changes';updateLimitedSaveState()},1600)}catch(error){submit.classList.remove('is-saving','is-saved');submit.classList.add('has-error');submit.textContent='Try again';feedback.textContent=error?.message||'The changes could not be saved. Please try again.'}finally{limitedForm.dataset.saving='0';dateInput.disabled=false;impactInput.disabled=false;if(!submit.classList.contains('is-saved')&&!submit.classList.contains('has-error'))updateLimitedSaveState()}};
