...
$hideAssignedColumn = !empty($hideAssignedColumn);
$taskTableColumnCount = $hideAssignedColumn ? 10 : 11;
$visibleBatchGroups = [];
if ($canManage) {
...
<section class="task-board" data-task-board data-task-kind="<?= htmlspecialchars((string) ($displayTaskKind ?? 'all'), ENT_QUOTES, 'UTF-8') ?>">
    ...
    <table class="dtb-board-table task-board-table">
        <colgroup><col class="dtb-col-select"><col class="dtb-col-name"><col class="dtb-col-actions"><?php if (!$hideAssignedColumn): ?><col class="dtb-col-assigned"><?php endif; ?><col class="dtb-col-priority"><col class="dtb-col-due"><col class="dtb-col-days"><col class="dtb-col-status"><col class="dtb-col-progress"><col class="dtb-col-completed"><col class="dtb-col-notes"></colgroup>
        <thead><tr><th class="dtb-select-cell"><input class="dtb-task-check dtb-task-check-all" type="checkbox" aria-label="Select all visible tasks"></th><th>Task</th><th>Details</th><?php if (!$hideAssignedColumn): ?><th>Assigned</th><?php endif; ?><th>Priority</th><th>Due</th><th>When Due</th><th>Status</th><th>Time Left</th><th>Completed</th><th>Notes</th></tr></thead>
        <tbody>
            <?php foreach ($displayTasks as $task): ?>
                ...
                $dueState = ['value'=>$timing['overdue']?'overdue':($savedStatus==='complete'?'complete':'upcoming'),'iso'=>(string)($timing['due_iso']??''),'title'=>$timing['due_label'].' — '.$whenDueOutcome,'label'=>$whenDueOutcome];
                $taskId = (int) $task['id'];
                ?>
                <tr class="dtb-task-row task-grid-row" data-task-row data-task-id="<?= $taskId ?>" data-deadline-state="<?= htmlspecialchars((string) ($dueState['value'] ?? 'normal'), ENT_QUOTES, 'UTF-8') ?>" data-saved-status="<?= htmlspecialchars($savedStatus, ENT_QUOTES, 'UTF-8') ?>" data-display-status="<?= htmlspecialchars($effective, ENT_QUOTES, 'UTF-8') ?>" data-task-name="<?= htmlspecialchars((string) $task['task_name'], ENT_QUOTES, 'UTF-8') ?>" data-task-assigned="<?= htmlspecialchars((string) ($task['assigned_name'] ?? 'Unassigned'), ENT_QUOTES, 'UTF-8') ?>" data-task-priority="<?= htmlspecialchars($priorities[$priorityKey] ?? 'Medium', ENT_QUOTES, 'UTF-8') ?>" data-task-status="<?= htmlspecialchars($groups[$effective] ?? ($statuses[$effective] ?? $effective), ENT_QUOTES, 'UTF-8') ?>">
                    ...
                    <?php if (!$hideAssignedColumn): ?><td><?= htmlspecialchars((string) ($task['assigned_name'] ?? 'Unassigned'), ENT_QUOTES, 'UTF-8') ?></td><?php endif; ?>
                    ...
                    <td class="task-progress-cell"><div class="task-countdown<?= $timing['overdue']?' is-overdue':'' ?><?= $savedStatus==='complete'?' is-complete':'' ?>" data-task-countdown data-task-due-at="<?= htmlspecialchars((string)($timing['due_iso']??''), ENT_QUOTES, 'UTF-8') ?>" role="timer" aria-live="off"><span data-task-countdown-value><?= $savedStatus==='complete'?'Completed':(!empty($timing['due_iso'])?'Calculating…':'No due date') ?></span></div></td>
                    <td data-task-completed><?= ($task['date_completed'] ?: $task['completed_at']) ? htmlspecialchars($hideAssignedColumn ? checklist_completed_date_label((string) ($task['date_completed'] ?: $task['completed_at'])) : checklist_date_label((string) ($task['date_completed'] ?: $task['completed_at'])), ENT_QUOTES, 'UTF-8') : ($savedStatus === 'complete' ? 'Completion time unavailable' : '-') ?></td>
                    <td><span class="task-notes-preview"><?= htmlspecialchars((string) ($task['completion_note'] ?: $task['notes'] ?: '—'), ENT_QUOTES, 'UTF-8') ?></span></td>
                </tr>
            <?php endforeach; ?>
            <?php if (!$displayTasks): ?><tr class="dtb-empty-row"><td colspan="<?= $taskTableColumnCount ?>"><?= htmlspecialchars($emptyTaskMessage, ENT_QUOTES, 'UTF-8') ?></td></tr><?php endif; ?>
        </tbody>
    </table>
    </div>
</section>
