...
try {
    ...
    foreach (ops_rows("SELECT id, task_name label, TIMESTAMPDIFF(HOUR,deadline,NOW()) age, assigned_employee_id employee_id,deadline due_at,priority FROM ops_checklist_tasks WHERE status NOT IN ('completed','complete','approved') AND deadline < NOW() AND created_at>=? AND deleted_at IS NULL ORDER BY deadline LIMIT 8",[$trackingStartSql]) as $row) $attention[]=['type'=>'task','severity'=>strtolower((string)$row['priority'])==='urgent'?'critical':'urgent','description'=>'Overdue task: '.$row['label'],'age_hours'=>(int)$row['age'],'overdue'=>kpi_business_duration((int)$row['age']*60),'due_at'=>$row['due_at'],'employee_id'=>(int)$row['employee_id'],'href'=>'checklists.php'];
    ...
    foreach($team as&$person){
        ...
        if($isPacker){$m=$packerMap[$id]??[];$assigned=(int)($m['assigned_items']??0);$completed=(int)($m['completed_items']??0);$durations=$packerDurationsByActor[$id]??[];$average=$durations?array_sum($durations)/count($durations):null;$person['card_type']='packer';$person['metrics']=[['label'=>'Packing Items Completed','value'=>$completed,'evidence'=>'ops_packing_tasks'],['label'=>'Packing Items Outstanding','value'=>(int)($m['outstanding_items']??0),'evidence'=>'ops_packing_tasks'],['label'=>'Workload Units','value'=>round((float)($m['workload_units']??0),2),'evidence'=>'ops_packing_tasks.workload_points','tooltip'=>'Calculated from quantity, weight or volume, product type, complexity and priority.'],['label'=>'Orders Packed','value'=>(int)($ordersPackedBy[$id]??0),'evidence'=>'order status activity'],['label'=>'Avg New → In Progress','value'=>kpi_business_duration($average),'evidence'=>'status history'],['label'=>'Completed versus Assigned','value'=>$assigned?$completed.' of '.$assigned.' completed':'Not calculated','numerator'=>$completed,'denominator'=>$assigned]];
        }else{$completed=(int)($ordersCompletedBy[$id]??0);$applicable=$completed+$orderFlow['outstanding'];$employeeDurations=[];foreach($orderRows as$order){foreach($eventsByOrder[(int)$order['id']]??[]as$event){$next=strtolower((string)($event['new_status']?:$event['action']));if(in_array($next,['completed','packed','verified','order_completed'],true)&&(int)($event['actor_user_id']??0)===$id&&strtotime((string)$event['occurred_at'])>=strtotime((string)$order['created_at'])){$employeeDurations[]=(strtotime((string)$event['occurred_at'])-strtotime((string)$order['created_at']))/60;break;}}}$person['card_type']='front_desk';$person['metrics']=[['label'=>'Orders Completed','value'=>$completed,'evidence'=>'order status activity'],['label'=>'Walk-in Orders Completed','value'=>(int)($walkInsCompletedBy[$id]??0),'evidence'=>'order status activity plus Walk-in identifier'],['label'=>'Orders Outstanding','value'=>$orderFlow['outstanding'],'evidence'=>'authoritative order status'],['label'=>'Avg New → Complete','value'=>kpi_business_duration($employeeDurations?array_sum($employeeDurations)/count($employeeDurations):null),'evidence'=>'status history'],['label'=>'Completed versus Applicable','value'=>$applicable?$completed.' of '.$applicable.' completed':'Not calculated','numerator'=>$completed,'denominator'=>$applicable],['label'=>'Website Updates Pending','value'=>$websitePending,'evidence'=>'ops_packing_tasks']];}
    }unset($person);
