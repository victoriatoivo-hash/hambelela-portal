...
try {
    ...
    $portalPaidCondition=ops_column_exists('ops_orders','portal_paid_confirmed')?"(CASE WHEN ops_orders.portal_paid_confirmed IS NULL THEN ops_orders.payment_status='paid' ELSE ops_orders.portal_paid_confirmed=1 END)":"ops_orders.payment_status='paid'";
    $paidTimestampParts=[];foreach(['portal_paid_decided_at','paid_updated_at','payment_updated_at']as$column)if(ops_column_exists('ops_orders',$column))$paidTimestampParts[]='ops_orders.'.$column;$paidTimestampExpr=$paidTimestampParts?'COALESCE('.implode(',',$paidTimestampParts).')':'NULL';
    $orders = kpi_scalar_row("SELECT COUNT(*) total, COALESCE(SUM(CASE WHEN {$paidRevenue} THEN total_amount ELSE 0 END),0) revenue, SUM(status IN ('completed','packed','verified')) completed_n, AVG(CASE WHEN status IN ('completed','packed','verified') AND completed_at IS NOT NULL AND completed_at >= created_at AND created_at >= ? THEN TIMESTAMPDIFF(MINUTE, created_at, completed_at) END) avg_minutes, AVG(CASE WHEN status IN ('completed','packed','verified') AND {$portalPaidCondition} AND {$paidTimestampExpr} IS NOT NULL AND completed_at IS NOT NULL AND completed_at>=created_at THEN TIMESTAMPDIFF(MINUTE,completed_at,GREATEST(completed_at,{$paidTimestampExpr})) END) settlement_minutes, AVG(CASE WHEN status IN ('completed','packed','verified') AND {$portalPaidCondition} AND {$paidTimestampExpr} IS NOT NULL AND completed_at IS NOT NULL AND completed_at>=created_at THEN TIMESTAMPDIFF(MINUTE,created_at,GREATEST(completed_at,{$paidTimestampExpr})) END) final_closure_minutes, SUM(CASE WHEN status IN ('completed','packed','verified') AND {$portalPaidCondition} AND {$paidTimestampExpr} IS NOT NULL AND completed_at IS NOT NULL AND completed_at>=created_at THEN 1 ELSE 0 END) settled_n, SUM(CASE WHEN status IN ('completed','packed','verified') AND completed_at IS NOT NULL AND completed_at >= created_at AND created_at >= ? AND TIMESTAMPDIFF(MINUTE, created_at, completed_at) <= ? * 60 THEN 1 ELSE 0 END) on_time_n FROM ops_orders WHERE created_at BETWEEN ? AND ?", [$rateFromSql, $rateFromSql, (float) ($settings['on_time_dispatch_hours'] ?? 6), $fromSql, $toSql]);
    ...
    $healthCards = [
        ...
        'settlement' => kpi_business_health_metric($orders['settlement_minutes'] !== null ? (float) $orders['settlement_minutes'] : null, (int)($orders['settled_n']??0), null, true, 'minutes'),
        'final_closure' => kpi_business_health_metric($orders['final_closure_minutes'] !== null ? (float) $orders['final_closure_minutes'] : null, (int)($orders['settled_n']??0), null, true, 'minutes'),
    ...
    $systemIssueMetrics=['total'=>0,'resolved_n'=>0,'reopened_n'=>0];
    if(ops_table_exists('system_issues'))$systemIssueMetrics=kpi_scalar_row("SELECT COUNT(*) total, SUM(internal_status='done' AND verified_at IS NOT NULL) resolved_n, SUM(workflow_stage='reopened' OR internal_status='reopened' OR EXISTS(SELECT 1 FROM system_issue_events ev WHERE ev.issue_id=system_issues.id AND ev.event_type IN ('issue_reopened','verification_failed'))) reopened_n FROM system_issues WHERE created_at BETWEEN ? AND ? AND deleted_at IS NULL AND duplicate_of_id IS NULL",[$rateFromSql,$toSql]);
    $systemIssueN=(int)($systemIssueMetrics['total']??0);$systemIssueResolved=(int)($systemIssueMetrics['resolved_n']??0);$systemIssueReopened=(int)($systemIssueMetrics['reopened_n']??0);
    $scores[]=['key'=>'system_issues','label'=>'System Issues','score'=>$systemIssueN>=5?max(0,round(80*$systemIssueResolved/max(1,$systemIssueN)+20*(1-$systemIssueReopened/max(1,$systemIssueN)))):null,'sample'=>$systemIssueN,'reason'=>$systemIssueResolved.' verified resolved · '.$systemIssueReopened.' reopened'];

    $communicationMetrics=['total'=>0,'missed_followups'=>0,'awaiting'=>0,'complaints'=>0];
    if(ops_table_exists('ops_whatsapp_conversations'))$communicationMetrics=kpi_scalar_row("SELECT COUNT(*) total, SUM(follow_up_at IS NOT NULL AND follow_up_at<NOW() AND status NOT IN ('resolved','abandoned')) missed_followups, SUM(status='awaiting_response' OR (last_customer_message_at IS NOT NULL AND (last_staff_response_at IS NULL OR last_customer_message_at>last_staff_response_at))) awaiting, SUM(complaint_flag=1) complaints FROM ops_whatsapp_conversations WHERE created_at BETWEEN ? AND ? AND status<>'abandoned'",[$rateFromSql,$toSql]);
    $communicationN=(int)($communicationMetrics['total']??0);$communicationMissed=(int)($communicationMetrics['missed_followups']??0);$communicationAwaiting=(int)($communicationMetrics['awaiting']??0);$communicationComplaints=(int)($communicationMetrics['complaints']??0);
    $scores[]=['key'=>'communications','label'=>'Customer Communications','score'=>$communicationN>=5?max(0,min(100,100-8*$communicationMissed-3*$communicationAwaiting-6*$communicationComplaints)):null,'sample'=>$communicationN,'reason'=>$communicationAwaiting.' awaiting response · '.$communicationMissed.' missed follow-ups'];

    $qualityMetrics=kpi_scalar_row("SELECT COUNT(*) total, SUM(LOWER(TRIM(COALESCE(status,''))) IN ('resolved','complete','completed','closed') OR NULLIF(TRIM(COALESCE(resolution,'')),'') IS NOT NULL) resolved_n, SUM(repeat_issue=1) repeat_n FROM ops_error_logs WHERE COALESCE(occurred_on,DATE(occurred_at),DATE(created_at),DATE(logged_at)) BETWEEN DATE(?) AND DATE(?) AND deleted_at IS NULL",[$rateFromSql,$toSql]);
    $qualityN=(int)($qualityMetrics['total']??0);$qualityResolved=(int)($qualityMetrics['resolved_n']??0);$qualityRepeat=(int)($qualityMetrics['repeat_n']??0);
    $scores[]=['key'=>'quality','label'=>'Error / Quality Resolution','score'=>$qualityN>=5?max(0,round(70*$qualityResolved/max(1,$qualityN)+30*(1-$qualityRepeat/max(1,$qualityN)))):null,'sample'=>$qualityN,'reason'=>$qualityResolved.' resolved · '.$qualityRepeat.' repeat issues'];
    ...
    foreach (ops_rows("SELECT id, COALESCE(error_title,category) label, TIMESTAMPDIFF(HOUR,logged_at,NOW()) age, employee_id,logged_at due_at,severity FROM ops_error_logs WHERE LOWER(TRIM(COALESCE(status,''))) NOT IN ('resolved','complete','completed','closed') AND NULLIF(TRIM(COALESCE(resolution,'')),'') IS NULL AND logged_at>=? AND deleted_at IS NULL ORDER BY logged_at LIMIT 8",[$trackingStartSql]) as $row) $attention[]=['type'=>'error','severity'=>strtolower((string)$row['severity'])==='critical'?'critical':'normal','description'=>'Unresolved confirmed quality error: '.$row['label'],'age_hours'=>(int)$row['age'],'overdue'=>kpi_business_duration((int)$row['age']*60),'due_at'=>$row['due_at'],'employee_id'=>(int)$row['employee_id'],'href'=>'errors.php'];
    ...
    foreach($team as&$person){
        $summaryMetric=(string)($person['card_type']??'')==='packer'?($person['metrics'][5]??null):($person['metrics'][4]??null);
        $person['summary_score']=isset($summaryMetric['numeric_value'])&&$summaryMetric['numeric_value']!==null?round(max(0,min(100,(float)$summaryMetric['numeric_value'])),1):null;
        $person['summary_label']=(string)($person['card_type']??'')==='packer'?'Completed assigned packing work':'Completed applicable order work';
    }unset($person);

    $liveActivity=[];
    if(ops_table_exists('ops_activity_logs')){
        foreach(ops_rows("SELECT l.id,l.employee_id,e.full_name employee,l.entity_type module,l.entity_id record_id,l.action,l.created_at occurred_at FROM ops_activity_logs l JOIN ops_employees e ON e.id=l.employee_id LEFT JOIN ops_roles r ON r.id=e.role_id WHERE l.created_at BETWEEN ? AND ? AND ".kpi_performance_employee_predicate('e','r')." ORDER BY l.created_at DESC,l.id DESC LIMIT 40",[$fromSql,$toSql])as$activity){
            $liveActivity[]=['id'=>'activity-'.(int)$activity['id'],'employee_id'=>(int)$activity['employee_id'],'employee'=>$activity['employee'],'module'=>$activity['module'],'record_id'=>(int)$activity['record_id'],'action'=>$activity['action'],'occurred_at'=>$activity['occurred_at']];
        }
    }
    if(ops_table_exists('kpi_status_events')){
        foreach(ops_rows("SELECT s.id,s.changed_by employee_id,e.full_name employee,s.module,s.record_id,s.old_status,s.new_status,s.changed_at occurred_at FROM kpi_status_events s JOIN ops_employees e ON e.id=s.changed_by LEFT JOIN ops_roles r ON r.id=e.role_id WHERE s.changed_at BETWEEN ? AND ? AND ".kpi_performance_employee_predicate('e','r')." ORDER BY s.changed_at DESC,s.id DESC LIMIT 40",[$fromSql,$toSql])as$activity){
            $liveActivity[]=['id'=>'status-'.(int)$activity['id'],'employee_id'=>(int)$activity['employee_id'],'employee'=>$activity['employee'],'module'=>$activity['module'],'record_id'=>(int)$activity['record_id'],'action'=>'status changed from '.($activity['old_status']?:'not set').' to '.($activity['new_status']?:'not set'),'occurred_at'=>$activity['occurred_at']];
        }
    }
    usort($liveActivity,static function(array$a,array$b):int{return strcmp((string)$b['occurred_at'],(string)$a['occurred_at']);});
    $liveActivity=array_slice($liveActivity,0,30);
