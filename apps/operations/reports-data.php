...
try {
    ...
    $staff = kpi_scalar_row("SELECT COUNT(*) scheduled FROM ops_employees e JOIN ops_roles r ON r.id=e.role_id WHERE e.status='active' AND r.role_key <> 'owner_admin' AND (e.hire_date IS NULL OR e.hire_date <= CURDATE())");
    ...
    $errorRow = kpi_scalar_row("SELECT COUNT(*) employee_errors FROM ops_error_logs WHERE cause='employee' AND logged_at BETWEEN ? AND ? AND deleted_at IS NULL", [$rateFromSql, $toSql]);
    ...
    $names=[]; foreach(ops_rows("SELECT id, full_name, r.name role_name, r.role_key FROM ops_employees e JOIN ops_roles r ON r.id=e.role_id WHERE e.status='active' AND r.role_key<>'owner_admin' ORDER BY e.full_name") as $row)$names[(int)$row['id']]=$row;
    ...
    $payload=['ok'=>true,'period'=>['key'=>$periodKey,'from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d'),'requested_from'=>$requestedFrom->format('Y-m-d'),'adoption_date'=>$adoption->format('Y-m-d'),'show_adoption_banner'=>$requestedFrom<$adoption],'cards'=>$healthCards,'scores'=>$scores,'attention'=>$attention,'team'=>$team,'trends'=>['orders'=>$orderTrend,'packing'=>$packingTrend]];
