...
try {
    ...
    foreach($team as&$person){
        ...
        if($isPacker){$m=$packerMap[$id]??[];$assigned=(int)($m['assigned_items']??0);$completed=(int)($m['completed_items']??0);$durations=$packerDurationsByActor[$id]??[];$average=$durations?array_sum($durations)/count($durations):null;$person['card_type']='packer';$person['metrics']=[['label'=>'Packing Items Completed','value'=>$completed,'evidence'=>'ops_packing_tasks'],['label'=>'Packing Items Outstanding','value'=>(int)($m['outstanding_items']??0),'evidence'=>'ops_packing_tasks'],['label'=>'Workload Units','value'=>round((float)($m['workload_units']??0),2),'evidence'=>'ops_packing_tasks.workload_points','tooltip'=>'Calculated from parsed quantity, package effort, weight or volume, size complexity and priority.'],['label'=>'Orders Packed','value'=>(int)($ordersPackedBy[$id]??0),'evidence'=>'order status activity'],['label'=>'Avg New → In Progress','value'=>kpi_business_duration($average),'evidence'=>'status history'],['label'=>'Completed versus Assigned','value'=>$assigned?$completed.' of '.$assigned.' completed':'Not calculated','numerator'=>$completed,'denominator'=>$assigned]];
