...
?>
<link rel="stylesheet" href="<?= htmlspecialchars(BASE_URL, ENT_QUOTES, 'UTF-8') ?>/assets/css/epi.css?v=10.0.0">
<main class="workspace module epi-dashboard" data-epi-dashboard data-owner="<?= $owner ? '1' : '0' ?>" data-viewer="<?= (int) ($user['id'] ?? 0) ?>" data-default-year="<?= $default->format('Y') ?>" data-default-month="<?= $default->format('n') ?>">
  <header class="epi-dashboard__header portal-header-with-status">
    <div><p class="eyebrow">Performance</p><h1>Employee Performance Intelligence</h1><p>Evidence-based performance, accountability and operational insight.</p></div>
    <?php if ($owner): ?><a class="btn-secondary" href="epi-scoring-performance.php">Scoring verification</a><?php endif; ?>
  </header>
  <?php if (!\Hambelela\EPI\Performance::enabled()): ?><div class="ops-alert">Owner preview only. The EPI feature flag is disabled, so the legacy KPI remains the primary performance page.</div><?php endif; ?>
  ...
  <dialog class="epi-evidence-dialog" data-epi-evidence><header><div><p class="eyebrow">Authoritative evidence</p><h2 data-epi-evidence-title>Evidence</h2></div><button type="button" data-epi-close aria-label="Close evidence">×</button></header><div data-epi-evidence-body></div></dialog>
  <div class="epi-presentation-controls" data-epi-presentation-controls hidden><button type="button" data-epi-slide="previous">Previous</button><span data-epi-slide-status></span><button type="button" data-epi-slide="next">Next</button><button type="button" data-epi-presentation-exit>Exit</button></div>
</main>
<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js" defer></script>
<script src="<?= htmlspecialchars(BASE_URL, ENT_QUOTES, 'UTF-8') ?>/assets/js/epi-dashboard.js?v=10.0.0" defer></script>
