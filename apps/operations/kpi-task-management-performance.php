...
function kpi_task_management_performance(array $employee, string $fromSql, string $toSql, array $holidays = []): array
{
    ...
    $processEvidenceRate = kpi_weighted_subscore([['share'=>50,'score'=>$checklistRate],['share'=>50,'score'=>$proofRate]]);
    $score = kpi_weighted_subscore([['share'=>35,'score'=>$onTime],['share'=>25,'score'=>$completionRate],['share'=>20,'score'=>$noteRate],['share'=>15,'score'=>$processEvidenceRate],['share'=>5,'score'=>$reworkRate]]);
    $stats = array_map('kpi_task_duration_stats', $durations);
    $metrics = [
        ['label'=>'Tasks Assigned','value'=>$counts['assigned'],'explanation'=>'Tasks released and assigned to this employee during the selected period.'],['label'=>'Tasks Completed','value'=>$counts['completed'],'explanation'=>'Assigned tasks whose authoritative current status or completion event is Complete.'],
        ...
        ['label'=>'Task Completion Rate','value'=>$completionRate,'format'=>'percent','numerator'=>$counts['completed_eligible'],'denominator'=>$counts['completion_eligible'],'explanation'=>'Reliable completed tasks divided by reliable completion-eligible tasks; owner-review records are excluded.'],
        ...
        ['label'=>'Completed Exactly at Due','value'=>$counts['exact_due'],'numerator'=>$counts['exact_due'],'denominator'=>$counts['deadline_eligible']],
        ['label'=>'Completed Late','value'=>$counts['late'],'numerator'=>$counts['late'],'denominator'=>$counts['deadline_eligible']],
        ['label'=>'On-time Completion Rate','value'=>$onTime,'format'=>'percent','numerator'=>$counts['on_time'],'denominator'=>$counts['deadline_eligible'],'explanation'=>'Tasks completed by their exact due timestamp divided by completed tasks with reliable deadline evidence.'],
        ['label'=>'Average Time Completed Early','value'=>$stats['early']['average'],'format'=>'minutes','denominator'=>$stats['early']['n']],
        ['label'=>'Median Time Completed Early','value'=>$stats['early']['median'],'format'=>'minutes','denominator'=>$stats['early']['n']],
        ['label'=>'Earliest Completion Before Due','value'=>$stats['early']['slowest'],'format'=>'minutes','denominator'=>$stats['early']['n']],
        ['label'=>'Pending Upcoming','value'=>$counts['pending_upcoming'],'explanation'=>'Open assigned tasks whose due time has not yet passed.'],['label'=>'Current Overdue Tasks','value'=>$counts['overdue'],'explanation'=>'Open assigned tasks whose exact due time has passed.'],
        ...
        ['label'=>'Shortest Completed-Late Duration','value'=>$stats['completed_late']['fastest'],'format'=>'minutes','denominator'=>$stats['completed_late']['n']],
        ['label'=>'Longest Completed-Late Duration','value'=>$stats['completed_late']['slowest'],'format'=>'minutes','denominator'=>$stats['completed_late']['n']],
    ...
    return ['rows'=>$rows,'metrics'=>$metrics,'counts'=>$counts,'floating_tasks'=>['allocated'=>$counts['floating_assigned'],'completed'=>$counts['floating_completed'],'on_time'=>$counts['floating_on_time'],'late'=>$counts['floating_late'],'outstanding'=>$counts['floating_outstanding'],'overdue'=>$counts['floating_overdue']],'duration_stats'=>$stats,'status_breakdown'=>$breakdown,'overdue_by_status'=>$byStatus,'risk_rows'=>$riskRows,
        ...
        'task_score'=>$score,'score_detail'=>['on_time'=>$onTime,'completion'=>$completionRate,'checklist'=>$checklistRate,'proof'=>$proofRate,'process_evidence'=>$processEvidenceRate,'notes'=>$noteRate,'rework'=>$reworkRate,'weights'=>['on_time'=>35,'completion'=>25,'notes'=>20,'process_evidence'=>15,'rework'=>5],'coverage'=>['assigned'=>$counts['assigned'],'deadline_eligible'=>$counts['deadline_eligible'],'timing_ack'=>$stats['ack']['n'],'timing_active'=>$stats['active']['n'],'timing_turnaround'=>$stats['turnaround']['n']]],
