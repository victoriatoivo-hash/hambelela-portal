...
<main id="<?= $tab === 'business-health' ? 'kpi-management' : ($tab === 'performance-reports' ? 'kpi-reports' : ($tab === 'business-activity' ? 'kpi-business-timeline-page' : 'kpi-management')) ?>" class="workspace module kpi-health-page" data-kpi-tab="<?= htmlspecialchars($tab, ENT_QUOTES, 'UTF-8') ?>">
    ...
    <?php elseif ($tab === 'business-health'): ?>
        ...
        <section class="kpi-chart-grid">
            ...
            <article class="kpi-health-panel"><div class="kpi-panel-heading"><div><p class="eyebrow">Completed packing work</p><h2>Packing Work by Employee</h2></div><div class="kpi-chart-toggle"><button type="button" class="active" data-kpi-chart-mode="raw">Completed Items</button><button type="button" data-kpi-chart-mode="weighted">Effort-Weighted Workload</button></div></div><div class="kpi-chart-context" data-kpi-packing-context>Completed packing items by employee and completion date.</div><div class="kpi-chart-frame"><canvas data-kpi-packing-chart></canvas></div></article>
        </section>
