...
function kpi_front_orders_dashboard_base(array $employee, string $fromSql, string $toSql, array $schedule, array $holidays, array $settings, array $leave = []): array
{
    ...
    foreach($base['rows'] as $index=>$row){$events=$eventsByOrder[(int)$row['id']]??[];$walkIn=(string)$row['walk_in']==='Yes';$created=!empty($row['new_order_at'])?new DateTimeImmutable((string)$row['new_order_at'],$zone):null;$readyEvent=kpi_front_order_ready_event($events,$created,$zone);$readyAt=!empty($row['ready_at'])?new DateTimeImmutable((string)$row['ready_at'],$zone):($readyEvent&&!empty($readyEvent['occurred_at'])?new DateTimeImmutable((string)$readyEvent['occurred_at'],$zone):null);$completed=!empty($row['completed_at'])?new DateTimeImmutable((string)$row['completed_at'],$zone):null;$clockStart=!empty($row['clock_start'])?new DateTimeImmutable((string)$row['clock_start'],$zone):($walkIn?$created:$readyAt);$duration=array_key_exists('duration_minutes',$row)?$row['duration_minutes']:($clockStart&&$completed&&$completed>=$clockStart?kpi_business_minutes($clockStart,$completed,$holidays):null);
        ...
        if(!empty($base['rows'][$index]['unresolved_attribution']))$base['rows'][$index]['result']='Historical timing evidence unavailable';
    ...
    foreach($base['rows'] as$row){
        if(empty($row['completed_at']))continue;$mode=kpi_front_order_normalize((string)($row['mode']??''));$completedAt=new DateTimeImmutable((string)$row['completed_at'],$zone);$startAt=null;$clock='';$scored=true;
        if(($row['walk_in']??'No')==='Yes'){$startAt=!empty($row['new_order_at'])?new DateTimeImmutable((string)$row['new_order_at'],$zone):null;$mode='walkin';$clock='New → Complete';}
        elseif(strpos($mode,'delivery')!==false){$startAt=!empty($row['payment_recorded_at'])?new DateTimeImmutable((string)$row['payment_recorded_at'],$zone):null;$mode='delivery';$clock='Paid → Complete';}
        elseif(strpos($mode,'courier')!==false){$startAt=!empty($row['payment_recorded_at'])?new DateTimeImmutable((string)$row['payment_recorded_at'],$zone):null;$mode='courier';$clock='Paid → Complete';}
        elseif(strpos($mode,'collection')!==false){$startAt=!empty($row['ready_at'])?new DateTimeImmutable((string)$row['ready_at'],$zone):null;$mode='collection';$clock='In Progress → Complete';$scored=false;}
        if(!$startAt||$completedAt<$startAt)continue;$minutes=kpi_business_minutes($startAt,$completedAt,$holidays);if(!isset($modeTimingGroups[$mode]))$modeTimingGroups[$mode]=['mode'=>$mode==='walkin'?'Walk-ins':ucfirst($mode),'clock'=>$clock,'scored'=>$scored,'durations'=>[]];$modeTimingGroups[$mode]['durations'][]=$minutes;
    }
    $modeTiming=[];foreach($modeTimingGroups as$group){$values=$group['durations'];sort($values,SORT_NUMERIC);$count=count($values);$mid=intdiv($count,2);$median=$count%2?$values[$mid]:(($values[$mid-1]+$values[$mid])/2);$modeTiming[]=['mode'=>$group['mode'],'clock'=>$group['clock'],'scored'=>$group['scored'],'count'=>$count,'average_minutes'=>round(array_sum($values)/$count,1),'median_minutes'=>$median,'fastest_minutes'=>$values[0],'slowest_minutes'=>$values[$count-1]];}
    $modeOrder=['Walk-ins'=>0,'Delivery'=>1,'Courier'=>2,'Collection'=>3];usort($modeTiming,static fn(array$a,array$b):int=>($modeOrder[$a['mode']]??99)<=>($modeOrder[$b['mode']]??99));
...
{
    $scoped = array_values(array_filter($rows, static fn(array $row): bool => $category === '*' || $row['front_desk_category'] === $category));
    $completedAll = array_values(array_filter($scoped, static fn(array $row): bool => !empty($row['completed_at'])));
    $pending = array_values(array_filter($scoped, static fn(array $row): bool => empty($row['completed_at'])));
    $eligible = array_values(array_filter($completedAll, static fn(array $row): bool => empty($row['unresolved_attribution']) && in_array($row['result'] ?? '', ['Completed on time','Completed late'], true)));
    $durations = array_values(array_filter(array_map(static fn(array $row) => $row['duration_minutes'], $eligible), static fn($value): bool => $value !== null));
...
function kpi_front_orders_evidence(array $employee, string $fromSql, string $toSql, array $schedule, array $holidays, array $settings, array $leave = []): array
{
    ...
    $rows=[];$durations=[];$outstandingAges=[];$counts=['walk_in'=>0,'front_desk_other'=>0,'front_desk_other_completed'=>0,'front_desk_other_pending'=>0,'completed'=>0,'on_time'=>0,'late'=>0,'outstanding'=>0,'in_progress_pending'=>0,'outstanding_overdue'=>0,'exceptions'=>0,'unresolved'=>0,'packer_only_removed'=>0];
    foreach ($orders as $order) {
        ...
        $walkIn=kpi_front_order_is_walk_in($order,$aliases);$packedByEmployee=(int)($order['assigned_packer_id']??0)===$employeeId;$completedByEmployee=$completedById===$employeeId;$currentStatus=kpi_front_order_normalize((string)$order['status']);$currentCompleted=in_array($currentStatus,['completed','complete'],true)||$completed!==null;$readyEvent=kpi_front_order_ready_event($events,$created,$zone);$readyAt=$readyEvent&&!empty($readyEvent['occurred_at'])?new DateTimeImmutable((string)$readyEvent['occurred_at'],$zone):null;$frontDeskEmployee=strpos((string)($employee['role_key']??''),'front_desk')!==false;
        ...
        elseif(!$currentCompleted&&$frontDeskEmployee&&($readyAt||$currentStatus==='inprogress'))$attribution=$readyAt?'Current Front Desk queue — Ready/In Progress transition':'Current Front Desk queue — authoritative In Progress status';
        else{$counts['packer_only_removed']++;continue;}
        $paidEvent=null;foreach($events as$event){if(kpi_front_order_normalize((string)($event['action']??''))==='paymentstatusupdated'&&kpi_front_order_normalize((string)($event['new_status']??''))==='paid'){$paidEvent=$event;break;}}$paid=kpi_front_order_normalize((string)$order['payment_status'])==='paid';$paidAtRaw=$order['payment_recorded_at']??($paidEvent['occurred_at']??null);$paidAt=$paidAtRaw?new DateTimeImmutable((string)$paidAtRaw,$zone):null;$paidBy=(string)($paidEvent['actor_name']??(($order['payment_recorded_by_employee_id']??null)?'Employee #'.(int)$order['payment_recorded_by_employee_id']:'Not recorded'));
        $modeKey=kpi_front_order_normalize((string)$order['fulfilment_mode']);$delivery=strpos($modeKey,'delivery')!==false;$collection=strpos($modeKey,'collection')!==false;$closeTarget=max(1,(int)($settings['front_orders_close_after_payment_minutes']??1440));$deadline=$created?kpi_front_order_due_at($created,$schedule,$holidayMap,$graceMinutes):null;$clockStart=$created;$clockBasis='New → Complete';$invalid=!$created||($completed&&$completed<$created);$duration=!$invalid&&$created&&$completed?kpi_business_minutes($created,$completed,$holidays):null;$exception=null;
        ...
        $pendingTiming=null;if(!$currentCompleted&&$currentStatus==='inprogress'){
            if($walkIn){$elapsed=$created?kpi_business_minutes($created,$now,$holidays):0;$remaining=max(0,$walkInTarget-$elapsed);$pendingTiming=['state'=>$elapsed>$walkInTarget?'overdue':'counting','label'=>$elapsed>$walkInTarget?'Walk-in completion overdue':'Walk-in completion window','elapsed_minutes'=>$elapsed,'remaining_minutes'=>$remaining,'target_minutes'=>$walkInTarget,'scored'=>true];}
            elseif($collection){$elapsed=$created?max(0,(int)(($now->getTimestamp()-$created->getTimestamp())/60)):0;$remaining=max(0,4320-$elapsed);$pendingTiming=['state'=>$elapsed>=4320?'review_due':'external_wait','label'=>$elapsed>=4320?'Cancellation review due':'Awaiting customer collection','elapsed_minutes'=>$elapsed,'remaining_minutes'=>$remaining,'target_minutes'=>4320,'scored'=>false];}
            elseif($delivery&&!$paid){$pendingTiming=['state'=>'paused','label'=>'Awaiting driver payment','elapsed_minutes'=>null,'remaining_minutes'=>null,'target_minutes'=>$closeTarget,'scored'=>false];}
            elseif($delivery&&$paidAt){$elapsed=kpi_business_minutes($paidAt,$now,$holidays);$remaining=max(0,$closeTarget-$elapsed);$pendingTiming=['state'=>$elapsed>$closeTarget?'overdue':'counting','label'=>$elapsed>$closeTarget?'Completion overdue after payment':'Paid — completion window','elapsed_minutes'=>$elapsed,'remaining_minutes'=>$remaining,'target_minutes'=>$closeTarget,'scored'=>true];}
            elseif($deadline){$remaining=max(0,(int)(($deadline->getTimestamp()-$now->getTimestamp())/60));$pendingTiming=['state'=>$now>$deadline?'overdue':'counting','label'=>$now>$deadline?'Completion overdue':'Completion window','elapsed_minutes'=>$created?kpi_business_minutes($created,$now,$holidays):null,'remaining_minutes'=>$remaining,'target_minutes'=>null,'scored'=>true];}
            else{$pendingTiming=['state'=>'review','label'=>'Timing evidence needs review','elapsed_minutes'=>null,'remaining_minutes'=>null,'target_minutes'=>null,'scored'=>false];}
        }
        if($walkIn)$counts['walk_in']++;else{$counts['front_desk_other']++;if($currentCompleted)$counts['front_desk_other_completed']++;else$counts['front_desk_other_pending']++;}if($currentCompleted)$counts['completed']++;else{$counts['outstanding']++;if($currentStatus==='inprogress')$counts['in_progress_pending']++;if($created)$outstandingAges[]=kpi_business_minutes($created,$now,$holidays);}if($unresolved)$counts['unresolved']++;if(!$unresolved&&$duration!==null)$durations[]=$duration;
        if($exception)$counts['exceptions']++;
        $rows[]=$order+['walk_in'=>$walkIn?'Yes':'No','walk_in_identifier'=>$walkIn?trim((string)($order['customer_contact']?:$order['customer_name']?:$order['order_type'])):'—','mode'=>ucfirst((string)$order['fulfilment_mode']),'packed_by'=>$order['packed_by_name']?:'Unassigned','status_before'=>$statusBefore?:'New','status_after'=>$statusAfter?:'Complete','new_order_at'=>$order['authoritative_created_at'],'ready_at'=>$readyAt?$readyAt->format('Y-m-d H:i:s'):null,'payment_recorded_at'=>$paidAt?$paidAt->format('Y-m-d H:i:s'):null,'payment_recorded_by'=>$paidBy,'completed_at'=>$completed?$completed->format('Y-m-d H:i:s'):null,'completed_by'=>$completedByName!==''?$completedByName:'Not recorded','clock_start'=>$clockStart?$clockStart->format('Y-m-d H:i:s'):null,'clock_basis'=>$clockBasis,'duration_minutes'=>$duration,'deadline'=>$deadline?$deadline->format('Y-m-d H:i:s'):null,'result'=>$result,'pending_timing'=>$pendingTiming,'exception_type'=>$exception,'attribution_source'=>$attribution,'evidence_id'=>$completionEvent?((string)($completionEvent['source_log']??'activity_log').' #'.(string)($completionEvent['source_event_id']??'')):'Current order record','evidence_count'=>count($events),'unresolved_attribution'=>$unresolved,'front_desk_category'=>$walkIn?'Walk-in':'Other Front Desk'];
    }
    ...
    return ['rows'=>$rows,'metrics'=>[
        ['label'=>'Front Desk Orders in Scope','value'=>count($rows),'explanation'=>$counts['walk_in'].' walk-ins assigned + '.$counts['front_desk_other_completed'].' other orders completed + '.$counts['front_desk_other_pending'].' orders currently in the Front Desk queue.'],['label'=>'Walk-ins Assigned','value'=>$counts['walk_in'],'explanation'=>'Orders identified by the configured walk-in marker and assigned to this employee on the Orders page.'],['label'=>'Other Front Desk Orders','value'=>$counts['front_desk_other'],'explanation'=>$counts['front_desk_other_completed'].' completed by this employee + '.$counts['front_desk_other_pending'].' currently pending with an authoritative In Progress status or recorded Front Desk handover.'],['label'=>'Completed Orders in Scope','value'=>$counts['completed'],'explanation'=>'In-scope orders whose authoritative current status or completion evidence is Complete.'],['label'=>'Completed On Time','value'=>$counts['on_time']],['label'=>'Completed Late','value'=>$counts['late']],['label'=>'Orders Still Pending Completion','value'=>$counts['in_progress_pending'],'explanation'=>'All current In Progress orders in the Front Desk queue that have not yet been completed, using the live order status even when the historical handover event is unavailable.'],['label'=>'Completion Compliance','value'=>$compliance,'format'=>'percent','status'=>$compliance===null?'unmeasured':'provisional','numerator'=>$counts['on_time'],'denominator'=>$completedEligible,'explanation'=>$counts['on_time'].' on time of '.$completedEligible.' completed orders with usable timing evidence; pending, external collection waiting and unresolved evidence are excluded.'],['label'=>'Unclear Historical Responsibility','value'=>$counts['unresolved']],['label'=>'Paid and Status Exceptions','value'=>$counts['exceptions'],'explanation'=>'Complete without Paid, Complete before the recorded payment, or missing payment audit evidence.']
    ],'walk_in_rate'=>$walkRate,'non_walk_rate'=>$nonWalkRate,'orders_score'=>$score,'walk_in_weight'=>$walkWeight,'non_walk_weight'=>$nonWalkWeight,'median_minutes'=>$median,'invalid_count'=>0,'review_count'=>$counts['unresolved'],'assisted_count'=>0,'walk_eligible'=>$walkCompleted,'non_walk_eligible'=>$nonWalkCompleted,'counts'=>$counts,'pending_orders'=>array_values(array_filter($rows,static fn(array$row):bool=>!empty($row['pending_timing']))),'methodology'=>'Walk-ins are reconciled from the configured walk-in marker and the employee assigned on Orders, with a two-business-hour New-to-Complete target. Non-walk-in pending work enters scope from its current authoritative In Progress status or a recorded Ready/In Progress handover; completed work requires the employee-authenticated Complete transition. Closed hours, Sundays and configured holidays are excluded. Every widget uses this same in-scope order set.','_events_by_order'=>$eventsByOrder];
}
