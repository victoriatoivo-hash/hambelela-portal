...
try {
    ...
    foreach ($employees as $person) {
        ...
        $website = performance_website_metrics($id,strpos($personRole,'packer')!==false,$fromSql,$toSql,$websiteLagTarget);
        ...
        $reports[] = ['id'=>$id,'name'=>$person['full_name'],'role'=>$person['role_name'],'role_key'=>$person['role_key'],'packing'=>['product_rows'=>(int)($packing['product_rows']??0),'packages'=>(int)($packing['packages']??0),'units'=>(float)($packing['units']??0),'workload_points'=>round((float)($packing['points']??0),1),'weight_grams'=>(float)($packing['grams']??0),'volume_ml'=>(float)($packing['millilitres']??0),'average_minutes'=>$packing['avg_minutes']===null?null:round((float)$packing['avg_minutes'],1),'timing_coverage'=>['numerator'=>(int)($packing['valid_timing']??0),'denominator'=>(int)($packing['product_rows']??0)],'requires_review'=>(int)($packing['review_rows']??0),'comparable_quantities'=>$quantityComparable,'quantity_variances'=>$quantityVariances,'quantity_accuracy'=>$quantityAccuracy],'orders'=>['packed'=>(int)($orders['packed_orders']??0)],'tasks'=>$tasks,'website'=>$website,'waybills'=>array_map('intval',$waybills),'quality'=>['eligible_work'=>$eligible,'verified_errors'=>$errorCount,'packing_variances'=>$quantityVariances,'packing_comparable'=>$quantityComparable,'accuracy'=>$quantityAccuracy,'status'=>$eligible>0?'measured':'not_measured']];
    }
    $ordersAnalysis=performance_orders_analysis($employees,$fromSql,$toSql);
    foreach($reports as$reportIndex=>$report){$id=(int)$report['id'];if(isset($ordersAnalysis['per_employee'][$id]))$reports[$reportIndex]['orders']=$ordersAnalysis['per_employee'][$id];}
