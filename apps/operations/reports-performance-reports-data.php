...
function performance_attendance_activity(array $employees, string $fromSql, string $toSql): array
{
    ...
    $events=kpi_unified_events($fromSql,$toSql);$byDay=[];$byHour=[];
    foreach($events as$event){$id=(int)($event['actor_user_id']??0);$section=(string)($event['section']??'');if(!isset($per[$id])||$section==='portal_activity')continue;$per[$id]['activity_count']++;$per[$id]['modules'][$section]=($per[$id]['modules'][$section]??0)+1;$stamp=strtotime((string)$event['occurred_at']);if($stamp!==false){$day=date('l',$stamp);$hour=(int)date('G',$stamp);$band=sprintf('%02d:00–%02d:59',$hour,$hour);$byDay[$id][$day]=($byDay[$id][$day]??0)+1;$byHour[$id][$band]=($byHour[$id][$band]??0)+1;}}
    ...
    unset($row);
    $available=kpi_activity_log_audit();$coverage=[];foreach($available as$source){$coverage[]=['source'=>$source['table'],'attributable'=>!empty($source['available'])&&!empty($source['actor']),'included'=>in_array($source['table'],['ops_activity_logs','kpi_status_events','ops_order_stage_events','hambelela_cashbook_log','hambelela_waybill_sla_log','kpi_activity_events'],true),'reason'=>empty($source['available'])?'Table unavailable':(empty($source['actor'])?'No employee actor field':(in_array($source['table'],['ops_activity_logs','kpi_status_events','ops_order_stage_events','hambelela_cashbook_log','hambelela_waybill_sla_log','kpi_activity_events'],true)?'Attributable events included':'Excluded from action count; reported by its dedicated metric'))];}
    return ['per_employee'=>$per,'coverage'=>$coverage,'source'=>'kpi_sessions plus deduplicated kpi_unified_events','event_count'=>array_sum(array_column($per,'activity_count'))];
}
