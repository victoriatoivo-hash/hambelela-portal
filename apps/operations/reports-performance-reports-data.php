...
function performance_attendance_activity(array $employees, string $fromSql, string $toSql, array $events): array
{
    $employeeNames=[];$per=[];
...
try {
    ...
    $unifiedEvents=performance_section_attempt($sectionErrors,'attendance',static fn()=>kpi_unified_events($fromSql,$toSql),[]);
    $attendanceActivity=performance_section_attempt($sectionErrors,'attendance',static fn()=>performance_attendance_activity($employees,$fromSql,$toSql,$unifiedEvents),['per_employee'=>[],'coverage'=>[],'source'=>'Unavailable','event_count'=>0]);
    $reports = [];
    foreach ($employees as $person) {
        $id = (int)$person['id'];
    ...
    foreach($reports as$reportIndex=>$reportRow){
        ...
        $reportEvents=array_values(array_filter($unifiedEvents,static fn(array$event):bool=>(int)($event['actor_user_id']??0)===$id));$reportSources=[];foreach($reportEvents as$reportEvent)$reportSources[$reportEvent['source_log']]=($reportSources[$reportEvent['source_log']]??0)+1;$reports[$reportIndex]['historical_evidence']=['events'=>count($reportEvents),'sources'=>$reportSources];
    }
