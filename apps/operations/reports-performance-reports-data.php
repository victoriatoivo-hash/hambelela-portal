...
function performance_orders_analysis(array $employees, string $fromSql, string $toSql): array
{
    ...
    $speedTotals=[];$completionTotals=[];$walkTotals=[];$walkOpenTotals=[];$businessTotal=0;$businessMeasured=0;$walkTotal=0;$walkPackedBy=[];$walkOpen=[];$walkLate=[];$walkSamples=[];$modeSamples=[];$modeObserved=[];$mobileObserved=[];$sequenceSamples=[];
    foreach($orders as$order){
        ...
        if($packer>0&&isset($per[$packer])){
            ...
            elseif($mode!=='other'){$per[$packer]['modes'][$mode]++;$per[$packer]['mode_total']++;if(count($modeSamples)<1)$modeSamples[]=['record_id'=>$id,'order_number'=>$order['order_number'],'packed_by'=>$employeeNames[$packer]??'Unassigned','mobile_value'=>$mobileRaw,'stored_mode'=>$modeRaw,'loaded_at'=>$loaded,'status'=>$order['status']];}
