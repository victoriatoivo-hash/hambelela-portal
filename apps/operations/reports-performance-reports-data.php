...
try {
    ...
    foreach($reports as$reportIndex=>$reportRow){
        ...
        $taskCompleted=(int)$reportRow['tasks']['completed'];$taskOpen=(int)$reportRow['tasks']['pending'];$taskOnTime=(int)$reportRow['tasks']['on_time'];$taskLate=(int)$reportRow['tasks']['completed_late'];$taskOverdue=(int)$reportRow['tasks']['open_overdue'];$taskN=$taskOnTime+$taskLate+$taskOverdue;$taskOnTimeRate=$taskN>0?100*$taskOnTime/$taskN:null;$taskReliability=$taskOpen>0?100*(1-$taskOverdue/$taskOpen):100.0;$taskScore=$taskOnTimeRate===null?null:.7*$taskOnTimeRate+.3*$taskReliability;$taskMetric=performance_scored_metric($taskScore,$taskN,['completed'=>$taskCompleted,'pending'=>$taskOpen,'overdue'=>$taskOverdue,'completed_late'=>$taskLate,'on_time_count'=>$taskOnTime,'on_time_denominator'=>$taskN,'on_time_percent'=>$taskOnTimeRate,'pending_reliability_percent'=>$taskReliability,'average_completed_late_minutes'=>$reportRow['tasks']['completed_overdue_average_minutes'],'total_completed_late_minutes'=>$reportRow['tasks']['completed_overdue_total_minutes'],'formula'=>'70% × on-time share + 30% × (1 − pending-overdue share)','working'=>'Tasks '.round((float)($taskScore??0),1).'% = 70% × on-time '.$taskOnTime.'/'.$taskN.' + 30% × (1 − overdue '.$taskOverdue.'/'.max(1,$taskOpen).'); completed late='.$taskLate.', avg late='.performance_task_duration_label($reportRow['tasks']['completed_overdue_average_minutes']).'; n='.$taskN,'definition'=>'On-time includes completed by deadline; denominator also includes completed-late and currently pending-overdue tasks'],array_slice($reportRow['tasks']['details'],0,100));
