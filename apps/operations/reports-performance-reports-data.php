...
function performance_scored_metric(?float $score, int $sample, array $supporting = [], array $details = [], bool $forceEligible = false): array
{
    ...
    $formula=(string)($supporting['formula']??'Corrected section query; no cached score');
    $working=(string)($supporting['working']??($formula.'; n='.$sample));
    return ['score'=>$eligible?round(max(0,min(100,$score)),1):null,'sample'=>$sample,'eligible'=>$eligible,'status'=>$eligible?'measured':'not_enough_data','formula'=>$formula,'working'=>$working,'supporting'=>$supporting,'details'=>$details];
}
...
{
    $points=0.0;$used=0.0;$missing=[];$measured=0;$core=count($weights);$totalWeight=(float)array_sum($weights);
    foreach($weights as$key=>$weight){$metric=$sections[$key]??null;if(!$metric||empty($metric['eligible'])){$missing[]=$key;continue;}$w=(float)$weight;$points+=(float)$metric['score']*$w;$used+=$w;$measured++;}
    $complete=$core>0&&$measured===$core&&$totalWeight>0;
    return ['score'=>$complete?round($points/$totalWeight,1):null,'measured_sections'=>$measured,'core_sections'=>$core,'coverage_label'=>$measured.' of '.$core.' sections measured','weight_used'=>round($used,1),'weights_renormalised'=>false,'excluded_sections'=>$missing,'status'=>$complete?'measured':'provisional'];
}
...
function performance_website_metrics(int $employeeId, bool $isPacker, string $fromSql, string $toSql, ?float $targetMinutes): array
{
    ...
    return ['source_type'=>'board_tick','required'=>(int)($summary['required']??0),'confirmed'=>$ticked,'unconfirmed'=>(int)($summary['unticked']??0),'ticked'=>$ticked,'unticked'=>(int)($summary['unticked']??0),'average_lag_minutes'=>$timed?round($lagTotal/$timed,1):null,'on_target_count'=>$onTarget,'on_target_total'=>$timed,'on_time_percent'=>$targetMinutes!==null&&$timed?round(100*$onTarget/$timed,1):null,'target_minutes'=>$targetMinutes,'timing_coverage'=>['attributable'=>$timed,'ticked'=>$ticked,'label'=>$timed.' of '.$ticked.' ticks attributable'],'attributable_ticks'=>$events,'updates'=>$ticked,'average_minutes'=>$timed?round($lagTotal/$timed,1):null];
}
...
try {
    ...
    $attributedUploadTotal=0;foreach($reports as$report){if(strpos((string)$report['role_key'],'packer')!==false)$attributedUploadTotal+=(int)$report['waybills']['uploaded'];}
    foreach($reports as$reportIndex=>$report){$reports[$reportIndex]['waybills']['upload_share_percent']=strpos((string)$report['role_key'],'packer')!==false&&$attributedUploadTotal>0?round(100*(int)$report['waybills']['uploaded']/$attributedUploadTotal,1):null;}
    ...
    $accuracyScored=(int)($settings['accuracy_scored']??0)===1;
    ...
    foreach($reports as$reportIndex=>$reportRow){
        ...
        if($isPacker){$orderN=(int)$reportRow['orders']['packed'];$measured=(int)$reportRow['orders']['speed_measured'];$speed=$reportRow['orders']['speed_minutes'];$orderTarget=$measured>0&&$speed!==null?max(0,min(100,100*(2-(float)$speed/$targetMinutes))):null;$compliance=$orderN>0?100*$measured/$orderN:null;$orderScore=performance_weighted_score([[$orderTarget,.7],[$compliance,.3]]);$orderMetric=performance_scored_metric($orderScore,$orderN,['orders_packed'=>$orderN,'completed'=>(int)$reportRow['orders']['completed'],'in_progress'=>(int)$reportRow['orders']['in_progress'],'new'=>(int)$reportRow['orders']['new'],'active_timing_count'=>$measured,'average_active_minutes'=>$speed,'timing_target_minutes'=>$targetMinutes,'timing_component_percent'=>$orderTarget,'timing_coverage'=>['measured'=>$measured,'total'=>(int)$reportRow['orders']['speed_total']],'completed_without_in_progress'=>$skippedInProgress,'status_compliance_percent'=>$compliance,'formula'=>'70% × loaded→In Progress timing + 30% × attributable timing coverage','working'=>'Orders '.round((float)($orderScore??0),1).'% = 70% × timing '.round((float)($orderTarget??0),1).'% (avg '.($speed===null?'not measured':round((float)$speed,1).'m').' vs '.$targetMinutes.'m target) + 30% × coverage '.$measured.'/'.$orderN.'; n='.$orderN,'timing_source'=>'Displayed order timestamp to first attributable In Progress activity event']);}
        ...
        $packingDone=(int)$reportRow['packing']['product_rows'];$packingComparable=(int)$reportRow['packing']['comparable_quantities'];$packingVariances=(int)$reportRow['packing']['quantity_variances'];$packingAccuracy=$packingComparable>0?100*($packingComparable-$packingVariances)/$packingComparable:($packingDone>0?100:null);$packingTiming=(int)$reportRow['packing']['timing_coverage']['numerator'];$packingTimingScore=$packingTiming>0?100:null;$packingBase=$accuracyScored?performance_weighted_score([[$packingTimingScore,.3],[$packingAccuracy,.7]]):$packingTimingScore;$packingMetric=performance_scored_metric($packingBase,$packingDone,['items_packed'=>$packingDone,'units_packed'=>$reportRow['packing']['units'],'weighted_points'=>$reportRow['packing']['workload_points'],'active_timing_count'=>$packingTiming,'average_active_minutes'=>$packingTiming>0?$reportRow['packing']['average_minutes']:null,'within_target_percent'=>$packingTimingScore,'quantity_comparable'=>$packingComparable,'quantity_variances'=>$packingVariances,'accuracy_percent'=>$packingAccuracy,'accuracy_scored'=>$accuracyScored?1:0,'accuracy_status'=>$accuracyScored?'score-affecting':'informational only','formula'=>$accuracyScored?'70% × verified accuracy + 30% × timing':'Timing only; accuracy is informational (accuracy_scored=0)','working'=>'Packing '.round((float)($packingBase??0),1).'% from '.$packingTiming.' timed of '.$packingDone.' items; accuracy '.($packingAccuracy===null?'not measured':round($packingAccuracy,1).'%').' is '.($accuracyScored?'included':'excluded'),'count_source'=>'ops_packing_tasks.date_completed + assigned_employee_id','timing_source'=>'ops_packing_tasks date_started/date_completed']);
        $taskCompleted=(int)$reportRow['tasks']['completed'];$taskOpen=(int)$reportRow['tasks']['pending'];$taskOnTime=(int)$reportRow['tasks']['on_time'];$taskLate=(int)$reportRow['tasks']['completed_late'];$taskOverdue=(int)$reportRow['tasks']['open_overdue'];$taskN=$taskOnTime+$taskLate+$taskOverdue;$taskOnTimeRate=$taskN>0?100*$taskOnTime/$taskN:null;$taskReliability=$taskOpen>0?100*(1-$taskOverdue/$taskOpen):100.0;$pendingLateMinutes=array_sum(array_map(static fn(array$x):float=>(float)($x['overdue_minutes']??0),$reportRow['tasks']['pending_overdue_details']??[]));$completedLateMinutes=(float)($reportRow['tasks']['completed_overdue_total_minutes']??0);$lateEventCount=$taskLate+$taskOverdue;$averageLateMinutes=$lateEventCount>0?($completedLateMinutes+$pendingLateMinutes)/$lateEventCount:0.0;$durationCompliance=max(0.0,min(100.0,100.0*(1.0-$averageLateMinutes/1440.0)));$taskScore=$taskOnTimeRate===null?null:.6*$taskOnTimeRate+.2*$taskReliability+.2*$durationCompliance;$taskMetric=performance_scored_metric($taskScore,$taskN,['completed'=>$taskCompleted,'pending'=>$taskOpen,'overdue'=>$taskOverdue,'completed_late'=>$taskLate,'on_time_count'=>$taskOnTime,'on_time_denominator'=>$taskN,'on_time_percent'=>$taskOnTimeRate,'pending_reliability_percent'=>$taskReliability,'average_completed_late_minutes'=>$reportRow['tasks']['completed_overdue_average_minutes'],'total_completed_late_minutes'=>$reportRow['tasks']['completed_overdue_total_minutes'],'pending_overdue_total_minutes'=>$pendingLateMinutes,'average_all_overdue_minutes'=>$averageLateMinutes,'overdue_duration_compliance_percent'=>$durationCompliance,'formula'=>'60% × on-time share + 20% × (1 − pending-overdue share) + 20% × verified overdue-duration compliance','working'=>'Tasks '.round((float)($taskScore??0),1).'% = 60% × on-time '.$taskOnTime.'/'.$taskN.' + 20% × (1 − overdue '.$taskOverdue.'/'.max(1,$taskOpen).') + 20% × duration '.round($durationCompliance,1).'% (avg '.performance_task_duration_label($averageLateMinutes).' overdue); completed late='.$taskLate.'; n='.$taskN,'definition'=>'On-time includes completed by deadline; denominator also includes completed-late and currently pending-overdue tasks; duration compliance reaches zero at an average verified overdue duration of 24 hours'],array_slice($reportRow['tasks']['details'],0,100));
    ...
    foreach($reports as$reportIndex=>$reportRow){if(strpos((string)$reportRow['role_key'],'packer')===false)continue;$packing=$reportRow['scored_sections']['packing'];$support=$packing['supporting'];$outputScore=$packerAverage>0?min(100,100*(float)$support['weighted_points']/$packerAverage):null;$tickRequired=(int)$reportRow['website']['required'];$tickCompliance=$tickRequired>0?100*(int)$reportRow['website']['confirmed']/$tickRequired:null;$support['output_vs_packer_average_percent']=$outputScore;$support['website_tick_compliance_percent']=$tickCompliance;$support['website_tick_weight_percent']=5;$support['website_tick_coverage']=$reportRow['website']['timing_coverage'];$components=[[$support['within_target_percent'],.38],[$outputScore,.285],[$tickCompliance,.05]];if($accuracyScored)$components[]=[$support['accuracy_percent'],.285];$score=performance_weighted_score($components);$support['accuracy_scored']=$accuracyScored?1:0;$support['accuracy_status']=$accuracyScored?'score-affecting':'informational only';$support['formula']=$accuracyScored?'Timing 38% + accuracy 28.5% + output 28.5% + board tick 5%':'Timing + output + board tick; available weights normalised across verified components';$support['working']='Packing '.round((float)($score??0),1).'%: timing '.round((float)($support['within_target_percent']??0),1).'%, output '.round((float)($outputScore??0),1).'%, tick '.round((float)($tickCompliance??0),1).'%; accuracy '.round((float)($support['accuracy_percent']??0),1).'% informational; n='.(int)$packing['sample'];$reports[$reportIndex]['scored_sections']['packing']=performance_scored_metric($score,(int)$packing['sample'],$support,$packing['details']);$reports[$reportIndex]['overall_score']=performance_overall_score($reports[$reportIndex]['scored_sections'],$configuredWeights['packer']??$defaultWeights['packer']);}
    $defaultBonusThreshold=max(0,min(100,(float)($settings['bonus_threshold']??75)));
    foreach($reports as$reportIndex=>$reportRow){$roleGroup=strpos((string)$reportRow['role_key'],'packer')!==false?'packer':'front_desk';$override=(float)($settings['bonus_threshold_'.$roleGroup]??0);$threshold=$override>0?max(0,min(100,$override)):$defaultBonusThreshold;$overall=$reportRow['overall_score'];$score=$overall['score'];$complete=($overall['status']??'provisional')==='measured';$below=[];foreach($reportRow['scored_sections'] as$name=>$metric){if(($metric['eligible']??false)&&is_numeric($metric['score'])&&(float)$metric['score']<$threshold)$below[]=['section'=>$name,'score'=>(float)$metric['score'],'gap'=>round($threshold-(float)$metric['score'],1)];}$verdict=!$complete?'Provisional — pending complete data':((float)$score>=$threshold?'Qualifies':'Does not qualify');$reports[$reportIndex]['reward']=['threshold'=>$threshold,'threshold_source'=>$override>0?'role_override':'default','score'=>$score,'verdict'=>$verdict,'gap'=>$score===null?null:round((float)$score-$threshold,1),'measured_sections'=>(int)($overall['measured_sections']??0),'total_sections'=>(int)($overall['core_sections']??count($reportRow['scored_sections'])),'below_threshold_sections'=>$below];}
