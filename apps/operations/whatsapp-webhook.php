...
if (!$ready) {
    ...
    echo 'WhatsApp webhook tables are not ready.';
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($mode === 'subscribe' && $expected !== '' && hash_equals($expected, $token)) {
        wa_log_webhook('verification', ['mode' => $mode], 'processed');
        header('Content-Type: text/plain');
        echo $challenge;
        exit;
    }

    wa_log_webhook('verification_failed', ['mode' => $mode], 'failed', 'Verify token mismatch.');
...
if (!is_array($payload)) {
    wa_log_webhook('invalid_payload', ['raw' => $raw], 'failed', 'Invalid JSON payload.');
...
try {
    wa_log_webhook('raw_event', $payload, 'received');
    ...
    wa_log_webhook('processing_error', $payload, 'failed', $e->getMessage());
