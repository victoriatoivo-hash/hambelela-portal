(function(){
  ...
  function unlock(){fields.disabled=false;form.setAttribute('aria-disabled','false');gate.hidden=true;endMarked=true;fetch('policy-action.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({csrf:cfg.csrf,action:'mark_end',version_id:cfg.versionId})}).catch(function(){})}
  ...
  form.addEventListener('submit',function(e){e.preventDefault();var m=document.getElementById('signatureMethod').value,err=document.getElementById('signatureError');err.hidden=true;if(!endMarked){err.textContent='Reach the acknowledgement section before signing.';err.hidden=false;return}if(m==='drawn'){if(!dirty){err.textContent='Draw your signature before continuing.';err.hidden=false;return}document.getElementById('signatureData').value=canvas.toDataURL('image/png')}if(!confirm('You are about to digitally sign this specific policy version.\n\nSelect OK to confirm this electronic signature and acknowledgement.'))return;var b=document.getElementById('signButton');b.disabled=true;b.textContent='Signing…';fetch(form.action,{method:'POST',body:new FormData(form)}).then(function(r){return r.json().then(function(j){if(!r.ok)throw new Error(j.error||'The acknowledgement could not be saved.');return j})}).then(function(j){allowExit=true;cfg.signed=true;form.outerHTML='<div class="signed-state"><h3>Policy signed & acknowledged</h3><p>Your acknowledgement was saved successfully.</p><a class="primary-btn" href="'+j.receipt_url+'">View acknowledgement receipt</a></div>';if(exitDialog)exitDialog.remove()}).catch(function(error){b.disabled=false;b.textContent='Review & confirm signature';err.textContent=error.message;err.hidden=false})});
})();
