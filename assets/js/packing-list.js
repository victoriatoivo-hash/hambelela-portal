(() => {
  ...
  function draftWorkloadTotals() {
    ...
    invoiceDraftRows.forEach((row) => {
      ...
      const current = totals.get(key) || { name, workload: 0, weight: 0, volume: 0, count: 0, rows: 0, manualRows: 0 };
      const physical = draftPhysical(row);
      current.name = name;
      ...
      <div class="draft-summary-grid">
        ${totals.map((item) => `
          <div class="draft-summary-card">
            ...
            <strong class="draft-physical-total">${formatPhysical('weight', item.weight)} &middot; ${formatPhysical('volume', item.volume)} &middot; ${formatPhysical('count', item.count)}</strong>
