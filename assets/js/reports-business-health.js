(() => {
  ...
  function renderOrdersOverview(overview, operationalScore, operationalScoreMessage, operationalComponents = []) {
    ...
    const timingMetric = (key, value, slowestReference = '') => `<span class="kpi-flow-metric is-${timingTone(key)}" title="${key === 'oldest_new' ? 'Age of the oldest order that is still in New and has not started packing.' : key === 'oldest_outstanding' ? 'Age of the oldest order currently inside the Front Desk responsibility stage.' : key === 'slowest' && slowestReference ? `Slowest qualifying order: ${escapeHtml(slowestReference)}` : 'Measured from authoritative order status timestamps.'}"><small>${escapeHtml(timingLabel(key))}</small><b>${escapeHtml(value || 'Not calculated')}</b>${key === 'slowest' && slowestReference ? `<em>${escapeHtml(slowestReference)}</em>` : ''}<i></i></span>`;
