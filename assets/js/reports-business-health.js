(() => {
  ...
  function renderOrdersOverview(overview, operationalScore, operationalScoreMessage, operationalComponents = []) {
    ...
    const timingMetric = (key, value) => `<span class="kpi-flow-metric is-${timingTone(key)}" title="${key === 'oldest_new' ? 'Age of the oldest order that is still in New and has not started packing.' : key === 'oldest_outstanding' ? 'Age of the oldest order that has not reached Complete.' : 'Measured from authoritative order status timestamps.'}"><small>${escapeHtml(timingLabel(key))}</small><b>${escapeHtml(value || 'Not calculated')}</b><i></i></span>`;
    const flowSection = (title, start, end, values, extra = '') => `<section class="kpi-flow-card"><header><h3>${escapeHtml(title)}</h3><div class="kpi-flow-track"><span>${escapeHtml(start)}</span><i></i><span>${escapeHtml(end)}</span></div></header><div class="kpi-flow-metrics">${Object.entries(values || {}).map(([key, value]) => timingMetric(key, value)).join('')}${extra}</div></section>`;
