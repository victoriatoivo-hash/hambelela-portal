(() => {
  ...
  function stackedBar(values, colours, cssClass) {
    ...
    const segments = Object.entries(values).map(([key, count]) => {
      ...
      return `<div class="ob-bar-segment summary-segment" style="flex:${numericCount / total};--summary-colour:${esc(colour)};color:${esc(colour)}" data-label="${esc(key)}" data-count="${esc(numericCount)}" data-total="${esc(total)}" data-percent="${esc(`${percent}%`)}" data-tooltip="${esc(tooltip)}" aria-label="${esc(tooltip)}" tabindex="0">
