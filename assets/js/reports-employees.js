(() => {
  ...
  async function readKpiJson(response){const raw=await response.text();if(!raw.trim())throw new Error(`The KPI server returned an empty response (${response.status}).`);if(!(response.headers.get('content-type')||'').toLowerCase().includes('application/json')){console.error('KPI response was not JSON:',raw.slice(0,500));throw new Error('The KPI server returned an invalid response.');}let data;try{data=JSON.parse(raw);}catch(error){console.error('KPI response could not be parsed:',raw.slice(0,500),error);throw new Error('The KPI server returned incomplete data.');}if(!response.ok||data.ok!==true)throw new Error(data.message||`KPI request failed (${response.status}).`);return data.data||data;}
  try {
  ...
  async function load(){
    ...
    try{
      ...
      q('[data-kpi-employees]').innerHTML=(data.employees||[]).map(person=>{const packer=String(person.role_key).includes('packer');const metrics=packer?[['Weighted points',shown(person.points)],['Items',shown(person.items)],['Open items',shown(person.open_items)]]:[['Orders processed',shown(person.orders_done)],['Website updates',shown(person.updates_done)],['Hours',shown(person.hours,value=>Number(value).toFixed(1))]];const bars=(sparks[person.id]||[]).map(value=>`<i style="height:${Math.max(4,Math.min(100,value*5))}%"></i>`).join('');const online=person.online===null?'Status unavailable':(Number(person.online)?'Online':'Offline');return `<a class="kpi-employee-card" href="kpi-employee.php?id=${person.id}&period=${encodeURIComponent(period.value)}${period.value==='custom'?`&date_from=${encodeURIComponent(from.value)}&date_to=${encodeURIComponent(to.value)}`:''}"><header><span class="kpi-person-dot ${Number(person.online)?'online':''}"></span><div><strong>${esc(person.full_name)}</strong><small>${esc(person.role_name)} · ${esc(online)}</small></div><b>${shown(person.hours,value=>`${Number(value).toFixed(1)}h`)}</b></header><div class="kpi-employee-metrics">${metrics.map(metric=>`<span><small>${esc(metric[0])}</small><strong>${esc(metric[1])}</strong></span>`).join('')}</div><div class="kpi-sparkline" title="14-day weighted output">${bars||'<span>—</span>'}</div></a>`;}).join('')||'<div class="kpi-empty-state">No active employees were returned.</div>';
      q('[data-kpi-error]').hidden=true;
    }catch(error){q('[data-kpi-employees]').innerHTML='<div class="kpi-empty-state">KPI data could not be loaded. Please try again.</div>';q('[data-kpi-error]').textContent=error.message;q('[data-kpi-error]').hidden=false;setCaption('Reporting period unavailable.');}
  }
