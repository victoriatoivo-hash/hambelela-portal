...
function accounts_purchase_payload(array $row): array
{
    ...
    return ['id'=>(int)$row['id'],'purchase_date'=>(string)$row['purchase_date'],'supplier'=>(string)$row['supplier'],'invoice_reference'=>(string)($row['invoice_reference']??''),'description'=>(string)$row['description'],'notes'=>(string)($row['notes']??''),'inclusive'=>(float)$row['amount_incl_vat'],'vat'=>(float)$row['vat_amount'],'exclusive'=>(float)$row['amount_excl_vat'],'vat_rate'=>(float)($row['vat_rate_used']??$row['vat_rate']),'vat_treatment'=>(string)$row['vat_treatment'],'calculation_source'=>(string)($row['calculation_source']??'inclusive'),'automatic_vat'=>(float)($row['automatic_vat_amount']??$row['vat_amount']),'automatic_exclusive'=>(float)($row['automatic_excl_vat']??$row['amount_excl_vat']),'manual_override'=>(bool)($row['manual_override']??false),'override_reason'=>(string)($row['override_reason']??''),'historical_back_capture'=>(bool)($row['historical_back_capture']??false),'review_status'=>(string)$row['review_status'],'review_note'=>(string)($row['review_note']??''),'entered_by'=>(string)$row['created_by_name'],'created_by'=>(int)$row['created_by'],'captured_at'=>(string)$row['created_at'],'updated_at'=>(string)$row['updated_at'],'can_edit'=>accounts_can_edit_purchase($row),'can_review'=>accounts_is_owner(),'can_delete'=>accounts_is_owner(),'attachments'=>array_map('accounts_attachment_payload',$stmt->fetchAll())];
}
