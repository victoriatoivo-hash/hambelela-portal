...
final class PerformanceScore
{
    ...
    public function employeeOptions():array{return$this->pdo->query("SELECT e.id,e.full_name,COALESCE(r.role_key,'') role_key FROM ops_employees e LEFT JOIN ops_roles r ON r.id=e.role_id WHERE e.status='active' AND COALESCE(r.role_key,'') NOT IN ('owner_admin','accountant') AND LOWER(CONCAT_WS(' ',e.full_name,e.email,COALESCE(r.role_key,''))) NOT REGEXP 'karina|kaarina|test|preview' ORDER BY e.full_name")->fetchAll(PDO::FETCH_ASSOC);}
    ...
    private function employee(int$id):array{$s=$this->pdo->prepare("SELECT e.id,e.full_name,e.email,e.status,COALESCE(r.role_key,'') role_key,'' department_key FROM ops_employees e LEFT JOIN ops_roles r ON r.id=e.role_id WHERE e.id=?");$s->execute([$id]);$r=$s->fetch(PDO::FETCH_ASSOC);if(!$r)throw new RuntimeException('Employee not found.');$identity=strtolower(implode(' ',[(string)$r['full_name'],(string)$r['email'],(string)$r['role_key']]));if((string)$r['status']!=='active'||in_array(strtolower((string)$r['role_key']),['owner_admin','accountant'],true)||preg_match('/karina|kaarina|test|preview/',$identity))throw new RuntimeException('This account is excluded from employee performance tracking.');return$r;}
    ...
    private function monthlyDetail(int$id):array{$s=$this->pdo->prepare('SELECT * FROM epi_scoring_monthly_scores WHERE id=?');$s->execute([$id]);$m=$s->fetch(PDO::FETCH_ASSOC)?:[];$s=$this->pdo->prepare('SELECT * FROM epi_employee_monthly_category_scores WHERE monthly_score_id=? ORDER BY id');$s->execute([$id]);$m['categories']=$s->fetchAll(PDO::FETCH_ASSOC);$s=$this->pdo->prepare('SELECT se.*,e.module evidence_module,e.reference_number,e.action evidence_action,e.action_description,e.occurred_at evidence_occurred_at,e.activity_source FROM epi_performance_score_events se LEFT JOIN epi_employee_evidence e ON e.evidence_uuid=se.evidence_uuid WHERE se.employee_id=? AND se.period_start=? AND se.period_end=? ORDER BY se.confirmation_status,se.created_at DESC');$s->execute([$m['employee_id'],$m['period_start'],$m['period_end']]);$m['events']=$s->fetchAll(PDO::FETCH_ASSOC);return$m;}
