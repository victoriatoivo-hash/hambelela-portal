...
final class PerformanceScore
{
    ...
    private function evidenceCount(int$id,string$f,string$t):int{$s=$this->pdo->prepare("SELECT COUNT(*) FROM epi_employee_evidence WHERE employee_id=? AND business_date BETWEEN ? AND ? AND verified=1 AND recording_mode<>'test'");$s->execute([$id,$f,$t]);return(int)$s->fetchColumn();}
