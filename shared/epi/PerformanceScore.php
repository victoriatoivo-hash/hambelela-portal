...
final class PerformanceScore
{
    ...
    public function employeeOptions():array{return$this->pdo->query("SELECT e.id,e.full_name,COALESCE(r.role_key,'') role_key FROM ops_employees e LEFT JOIN ops_roles r ON r.id=e.role_id WHERE e.status='active' AND LOWER(CONCAT_WS(' ',e.full_name,e.email,COALESCE(r.role_key,''))) NOT REGEXP 'karina|kaarina|test|preview' ORDER BY e.full_name")->fetchAll(PDO::FETCH_ASSOC);}
