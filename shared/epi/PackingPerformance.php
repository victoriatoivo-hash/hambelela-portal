...
final class PackingPerformance
{
    ...
    public function employeeOptions(): array{$q=$this->pdo->query("SELECT DISTINCT e.id,e.full_name FROM ops_employees e JOIN epi_employee_evidence ev ON ev.employee_id=e.id AND ev.module IN ('Packing List','Orders') ORDER BY e.full_name");return $q->fetchAll(PDO::FETCH_ASSOC)?:[];}
