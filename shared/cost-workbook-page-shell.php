...
function cw_page_begin(string $key, array $period, ?string $bootError): void
{
    ...
    <?php if($key==='transport-costs'): ?><link rel="stylesheet" href="<?= BASE_URL ?>/assets/css/cost-workbook-transport-costs.css?v=<?= (int) filemtime(BASE_PATH.'/assets/css/cost-workbook-transport-costs.css') ?>"><?php endif; ?>
    ...
    <?php if(in_array($key,['overview','size-conversions','supplier-invoices','transport-costs','packaging-costs','landed-product-costs'],true)): ?><link rel="stylesheet" href="<?= BASE_URL ?>/assets/css/accounts-dashboard.css?v=<?= (int) filemtime(BASE_PATH.'/assets/css/accounts-dashboard.css') ?>"><link rel="stylesheet" href="<?= BASE_URL ?>/assets/css/cost-workbook-landing.css?v=<?= (int) filemtime(BASE_PATH.'/assets/css/cost-workbook-landing.css') ?>"><?php endif; ?>
    <main class="workspace cw cw-page cost-workbook-page <?= in_array($key,['overview','size-conversions','supplier-invoices','transport-costs','packaging-costs','landed-product-costs'],true)?'cost-workbook-app accounts-workspace':'' ?>" id="costWorkbook" data-page="<?= htmlspecialchars($key, ENT_QUOTES, 'UTF-8') ?>" data-api="cw-api.php" data-phase2-api="cw-phase2-api.php" data-csrf="<?= htmlspecialchars(cw_csrf_token(), ENT_QUOTES, 'UTF-8') ?>" data-year="<?= (int) $period['year'] ?>" data-month="<?= (int) $period['month'] ?>">
      ...
      <?php elseif($key==='transport-costs'): ?><div class="transport-costs__toolbar"><a href="<?= cw_page_url('workbook.php',$period) ?>">← Back to Cost Workbook</a></div><header class="accounts-hero cw-workspace-header transport-costs__header"><div><p class="eyebrow">Cost Workbook</p><h1>Transport Cost Allocation</h1><p><?= htmlspecialchars($page['description'], ENT_QUOTES, 'UTF-8') ?></p></div><div class="transport-costs__actions"><button type="button" class="portal-button portal-button--secondary" data-transport-manual>Add Transport Manually</button><a class="portal-button portal-button--primary" href="#transportUpload">Upload Transport Invoice</a></div></header>
      ...
      <?php if(!in_array($key,['overview','size-conversions','supplier-invoices'],true)): ?><nav class="cw-period-nav" aria-label="Cost Workbook period"><a href="<?= cw_page_url($page['route'], ['year'=>(int)(new DateTimeImmutable($period['id'].'-01'))->modify('-1 month')->format('Y'),'month'=>(int)(new DateTimeImmutable($period['id'].'-01'))->modify('-1 month')->format('n')]) ?>" aria-label="Previous month">←</a><?php for($m=1;$m<=12;$m++): ?><a class="<?= $m===$period['month']?'is-active':'' ?>" <?= $m===$period['month']?'aria-current="date"':'' ?> href="<?= cw_page_url($page['route'], ['year'=>$period['year'],'month'=>$m]) ?>"><?= htmlspecialchars(DateTimeImmutable::createFromFormat('!m',(string)$m)->format('M'),ENT_QUOTES,'UTF-8') ?></a><?php endfor; ?><a href="<?= cw_page_url($page['route'], ['year'=>(int)(new DateTimeImmutable($period['id'].'-01'))->modify('+1 month')->format('Y'),'month'=>(int)(new DateTimeImmutable($period['id'].'-01'))->modify('+1 month')->format('n')]) ?>" aria-label="Next month">→</a></nav>
...
function cw_page_end(array $scripts = []): void
{
    ...
    foreach ($scripts as $script) {
        $defer = (strpos($script, 'supplier-invoices-v2.js') === 0 || strpos($script, 'transport-costs.js') === 0 || strpos($script, 'packaging-costs.js') === 0 || strpos($script, 'landed-product-costs.js') === 0) ? '' : ' defer';
