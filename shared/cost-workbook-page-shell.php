...
function cw_page_begin(string $key, array $period, ?string $bootError): void
{
    ...
    <?php if(in_array($key,['overview','size-conversions','supplier-invoices','transport-costs','packaging-costs'],true)): ?><link rel="stylesheet" href="<?= BASE_URL ?>/assets/css/accounts-dashboard.css?v=<?= (int) filemtime(BASE_PATH.'/assets/css/accounts-dashboard.css') ?>"><link rel="stylesheet" href="<?= BASE_URL ?>/assets/css/cost-workbook-landing.css?v=<?= (int) filemtime(BASE_PATH.'/assets/css/cost-workbook-landing.css') ?>"><?php endif; ?>
    <main class="workspace cw cw-page cost-workbook-page <?= in_array($key,['overview','size-conversions','supplier-invoices','transport-costs','packaging-costs'],true)?'cost-workbook-app accounts-workspace':'' ?>" id="costWorkbook" data-page="<?= htmlspecialchars($key, ENT_QUOTES, 'UTF-8') ?>" data-api="cw-api.php" data-phase2-api="cw-phase2-api.php" data-csrf="<?= htmlspecialchars(cw_csrf_token(), ENT_QUOTES, 'UTF-8') ?>" data-year="<?= (int) $period['year'] ?>" data-month="<?= (int) $period['month'] ?>">
...
function cw_page_end(array $scripts = []): void
{
    ...
    foreach ($scripts as $script) {
        $defer = (strpos($script, 'supplier-invoices-v2.js') === 0 || strpos($script, 'transport-costs.js') === 0 || strpos($script, 'packaging-costs.js') === 0) ? '' : ' defer';
