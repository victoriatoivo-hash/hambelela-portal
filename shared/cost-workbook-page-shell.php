...
function cw_page_routes(): array
{
    return [
        ...
        'purchases' => ['label' => 'Purchases', 'route' => 'purchases.php', 'title' => 'Purchases', 'description' => 'Upload and review supplier invoices for the selected period.'],
        'invoice-review' => ['label' => 'Invoice Review', 'route' => 'invoice-review.php', 'title' => 'Invoice Review', 'description' => 'Complete invoice lines, validation and approval for the selected period.'],
...
function cw_page_begin(string $key, array $period, ?string $bootError): void
{
    ...
    <main class="workspace cw cw-page" id="costWorkbook" data-page="<?= htmlspecialchars($key, ENT_QUOTES, 'UTF-8') ?>" data-api="cw-api.php" data-phase2-api="cw-phase2-api.php" data-csrf="<?= htmlspecialchars(cw_csrf_token(), ENT_QUOTES, 'UTF-8') ?>" data-year="<?= (int) $period['year'] ?>" data-month="<?= (int) $period['month'] ?>">
      <header class="cw-hero"><div><p class="cw-eyebrow">Hambelela Organic · <?= htmlspecialchars($period['id'], ENT_QUOTES, 'UTF-8') ?></p><h1><?= htmlspecialchars($page['title'], ENT_QUOTES, 'UTF-8') ?></h1><p><?= htmlspecialchars($page['description'], ENT_QUOTES, 'UTF-8') ?></p></div><div class="cw-hero-actions"><a class="cw-historical-link" href="<?= cw_page_url('historical-cost-records.php', $period) ?>">View Historical Cost Records</a><?php if(in_array($key,['purchases','invoice-review'],true)): ?><button class="cw-btn cw-primary" data-open-upload>Upload invoice</button><?php endif; ?></div></header>
      <div class="cw-alert" role="status">WooCommerce Cost of Goods Sold is not enabled. Cost publishing is unavailable. Profitability figures are private Cost Workbook previews and are not published to WooCommerce.</div>
      ...
      <nav class="cw-steps" aria-label="Workbook sections"><?php foreach($routes as $routeKey=>$item): ?><a class="cw-section-link <?= $routeKey===$key?'is-active':'' ?>" <?= $routeKey===$key?'aria-current="page"':'' ?> href="<?= cw_page_url($item['route'],$period) ?>"><span><?= htmlspecialchars(substr($item['label'],0,1),ENT_QUOTES,'UTF-8') ?></span><?= htmlspecialchars($item['label'],ENT_QUOTES,'UTF-8') ?></a><?php endforeach; ?></nav>
    <?php
}
