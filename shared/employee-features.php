...
function portal_feature_permissions(): array
{
    ...
    return [
        'owner_admin' => $ownerFeatures,
        'front_desk_admin' => ['packing_list'],
        'front_desk_admin_employee' => ['packing_list'],
        'packer' => ['packing_list'],
        'packer_production_staff' => ['packing_list'],
        'supervisor_manager' => ['packing_list'],
    ];
}
...
function portal_role_can_access_feature(string $roleKey, string $featureKey): bool
{
    ...
    $normalisedRole = normalise_portal_role($roleKey);
    return in_array($featureKey, $permissions[$normalisedRole] ?? [], true);
}

function employee_can_access_feature(string $featureKey, ?array $user = null): bool
{
    $account = $user ?? current_user();
    $role = (string) ($account['role_key'] ?? $account['role'] ?? 'guest');
    return portal_role_can_access_feature($role, $featureKey);
}
...
function employee_feature_for_request(string $scriptName): ?array
{
    ...
    $routes = [
        ...
        '/apps/operations/orders.php'=>['inventory','Inventory'], '/apps/operations/checklists.php'=>['task_management','Task Management'],
        ...
        '/apps/operations/reports.php'=>['kpi_dashboard','KPI Dashboard'], '/apps/operations/my-account.php'=>['settings','Settings'],
