...
function siw_stages(): array {
    return [
        ...
        'testing'=>['label'=>'Testing','employee'=>'testing','step'=>4,'next'=>'Explicitly confirm testing, then record deployment if required.'],
        'ready_for_verification'=>['label'=>'Ready for Verification','employee'=>'testing','step'=>5,'next'=>'Repeat the action that failed and confirm the result.'],
        'done'=>['label'=>'Done','employee'=>'done','step'=>5,'next'=>'The owner verified that the problem is fixed.'],
...
function siw_command_registry(): array {
    return [
        ...
        'testing_passed'=>['from'=>['testing'],'to'=>'testing','label'=>'Testing Passed','pending'=>'Saving…','event'=>'testing_passed','message'=>'explicitly confirmed testing passed.','primary'=>true],
        'record_deployment'=>['from'=>['testing'],'to'=>'testing','label'=>'Record Deployment Result','pending'=>'Saving…','event'=>'deployment_recorded','message'=>'recorded the deployment result.','primary'=>true],
...
function siw_permitted_actions(string $stage,int $issueId=0): array {
    ...
    if($stage==='testing'&&$issueId>0){$attempt=siw_latest_attempt($issueId);if($attempt&&$attempt['testing_completed_at']){unset($allowed['testing_passed']);if((int)$attempt['deployment_required']!==1)unset($allowed['record_deployment']);}else unset($allowed['record_deployment']);}
