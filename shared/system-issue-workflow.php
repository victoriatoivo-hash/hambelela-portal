...
function siw_execute(int $issueId,string $command,string $expectedStage,int $expectedVersion,string $idempotencyKey,string $note='',array $details=[]): array {
    ...
    try{
        ...
        $issue=ops_rows('SELECT * FROM system_issues WHERE id=? FOR UPDATE',[$issueId])[0]??null;if(!$issue)throw new OutOfBoundsException('issue_not_found');$stage=siw_normalise_stage($issue);if($stage==='needs_information'&&!siw_has_blocking_employee_request($issue,true))$stage='brief_ready';$version=(int)($issue['workflow_version']??1);if($stage!==$expectedStage||$version!==$expectedVersion)throw new UnexpectedValueException('stale_workflow');if(!in_array($stage,$rule['from'],true))throw new DomainException('invalid_transition');
        ...
        $fresh=ops_rows('SELECT * FROM system_issues WHERE id=? LIMIT 1',[$issueId])[0]??null;if(!$fresh)throw new OutOfBoundsException('issue_not_found');$response=siw_view($fresh);$responseAttempt=siw_latest_attempt($issueId);$primaryNext=['fix_in_progress'=>'testing','deployment'=>'ready_for_verification','ready_for_verification'=>'done','reopened'=>'fix_in_progress'][$response['workflow_stage']]??$response['workflow_stage'];if($response['workflow_stage']==='testing')$primaryNext=(int)($responseAttempt['deployment_required']??0)===1?'deployment':'ready_for_verification';$response['previous_stage']=$stage;$response['current_stage']=$response['workflow_stage'];$response['next_stage']=$primaryNext;$response['status']=$response['internal_status'];$response['workflow_status']=$response['internal_status'];$response['transition_completed']=$response['workflow_stage']===$to&&$response['workflow_version']===$newVersion;$response['message']=$command==='record_codex_result'?'Repair recorded — moved to Testing.':($command==='testing_passed'?('Testing recorded — moved to '.($response['form_mode']==='record_deployment'?'Deployment':'Verification').'.'):($target['label'].' saved successfully.'));$response['activity_event']=['title'=>ucwords(str_replace('_',' ',$rule['event'])),'message'=>$message,'created_at_display'=>date('j M Y · H:i')];$encoded=json_encode($response,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);db()->prepare('INSERT INTO system_issue_workflow_actions(issue_id,idempotency_key,command,response_json) VALUES(?,?,?,?)')->execute([$issueId,$idempotencyKey,$command,$encoded]);db()->commit();return $response;
