...
function siw_stages(): array {return [
 ...
 'repair_queue_failed'=>['label'=>'Repair queue failed','employee'=>'under_review','step'=>3,'next'=>'Retry the existing queue job or copy the approved Codex brief.'],
...
function siw_command_registry(): array {return [
 ...
 'retry_queue'=>['from'=>['approved','repair_queue_failed'],'to'=>'approved','label'=>'Retry Queue','pending'=>'Retrying…','event'=>'repair_queue_retried','message'=>'retried the existing controlled repair queue job.','primary'=>true],
];}
...
function siw_automation_configured(): bool {return defined('SYSTEM_ISSUES_WORKFLOW_WEBHOOK')&&defined('SYSTEM_ISSUES_WORKFLOW_SECRET')&&SYSTEM_ISSUES_WORKFLOW_WEBHOOK!==''&&SYSTEM_ISSUES_WORKFLOW_SECRET!=='';}
function siw_queue_state(array $issue): array {
 $issueId=(int)($issue['id']??0);$job=$issueId>0?(ops_rows('SELECT id,status,attempts,last_error,available_at,created_at,processed_at FROM system_issue_workflow_outbox WHERE issue_id=? ORDER BY id DESC LIMIT 1',[$issueId])[0]??null):null;$configured=siw_automation_configured();$age=$job?max(0,time()-strtotime((string)$job['created_at'])):0;$delayed=$job&&in_array($job['status'],['pending','retry'],true)&&$age>=60;
 return ['automation_configured'=>$configured,'queue_job_id'=>$job?(int)$job['id']:null,'queue_status'=>$job?(string)$job['status']:'missing','queue_attempts'=>$job?(int)$job['attempts']:0,'queue_created_at'=>$job['created_at']??null,'queue_available_at'=>$job['available_at']??null,'queue_processed_at'=>$job['processed_at']??null,'queue_last_error'=>$job&&!empty($job['last_error'])?'The repair integration could not process this job.':null,'queue_delayed'=>(bool)$delayed];
}
function siw_view(array $issue): array {
 ...
 $result=['issue_id'=>(int)($issue['id']??0),'internal_status'=>$blocking?'needs_information':($stage==='awaiting_owner_approval'?'brief_ready':$stage),'workflow_stage'=>$stage,'workflow_version'=>(int)($issue['workflow_version']??1),'workflow_label'=>$blocking?'Waiting for employee information':$definition['label'],'employee_status'=>$blocking?'needs_information':$definition['employee'],'employee_status_label'=>system_issue_status_label($blocking?'needs_information':$definition['employee']),'approval_allowed'=>!$blocking&&$stage==='awaiting_owner_approval','message'=>'Workflow state loaded.','next_step'=>$blocking?'Approval becomes available after the required information is received.':$definition['next'],'next_required_action'=>$blocking?'Wait for the requested employee information.':$definition['next'],'progress_step'=>$definition['step'],'permitted_actions'=>$actions,'blocking_employee_request'=>$blocking];
 if(in_array($stage,['approved','repair_queue_failed','codex_queued','codex_running'],true)){$result=array_merge($result,siw_queue_state($issue));if(!$result['automation_configured']){$result['next_step']='Repair approved, but automated repair processing has not been configured.';$result['next_required_action']=$result['next_step'];$result['permitted_actions']=[['command'=>'copy_codex_brief','label'=>'Copy Codex Brief','pending_label'=>'Copying…','primary'=>true]];}elseif($result['queue_delayed']){$result['next_step']='The repair was approved, but processing has not started.';$result['next_required_action']=$result['next_step'];$result['permitted_actions']=[['command'=>'retry_queue','label'=>'Retry Queue','pending_label'=>'Retrying…','primary'=>true],['command'=>'copy_codex_brief','label'=>'Copy Codex Brief','pending_label'=>'Copying…','primary'=>false]];}}
 return $result;
}
...
function siw_execute(int $issueId,string $command,string $expectedStage,int $expectedVersion,string $idempotencyKey,string $note=''): array {
 ...
 $actor=(int)(current_user()['id']??0);$actorName=(string)(current_user()['name']??'Owner');$rule=siw_command_registry()[$command];db()->beginTransaction();try{
  ...
  if($command==='retry_queue'){$job=ops_rows("SELECT * FROM system_issue_workflow_outbox WHERE issue_id=? AND event_type='approve_repair' ORDER BY id DESC LIMIT 1 FOR UPDATE",[$issueId])[0]??null;if(!$job)throw new LogicException('queue_job_missing');db()->prepare("UPDATE system_issue_workflow_outbox SET status='retry',attempts=attempts+1,available_at=NOW(),last_error=NULL WHERE id=?")->execute([(int)$job['id']]);system_issue_event($issueId,'repair_queue_retried',$stage,$stage,$actorName.' retried the existing controlled repair queue job.',['outbox_id'=>(int)$job['id']]);$result=siw_view($issue);$result['message']='The existing repair queue job was made available for retry.';$result['activity_event']=['title'=>'Repair Queue Retried','message'=>$actorName.' retried the existing controlled repair queue job.','created_at_display'=>date('j M Y · H:i')];$encoded=json_encode($result,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);db()->prepare('INSERT INTO system_issue_workflow_actions(issue_id,idempotency_key,command,response_json) VALUES(?,?,?,?)')->execute([$issueId,$idempotencyKey,$command,$encoded]);db()->commit();return $result;}
