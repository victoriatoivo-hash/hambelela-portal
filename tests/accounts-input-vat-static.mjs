...
assert.equal((liveEntry.match(/<select\b/g) || []).length, 8, 'cache-safe Input VAT template select inventory must stay explicit');
assert.equal((liveEntry.match(/data-portal-custom-select\b/g) || []).length, 8, 'cache-safe Input VAT template must activate every portal dropdown');
assert.equal((liveEntry.match(/data-portal-select-variant="input-vat"/g) || []).length, 8, 'cache-safe Input VAT template must retain the Input VAT theme variant');
