...
for(const label of ['Items Assigned','Items Completed','Units Requested','Units Packed','Exact Quantity Accuracy','Priority Adherence','Average Queue Time','Median Active Packing Time','Average Total Turnaround','Solid Units Packed','Liquid Units Packed','Large-Volume Items','Needs Label','Needs Correction','Packer Website Updates Completed','Items Requiring Review'])assert.match(service,new RegExp(label));
for(const label of ['Packing List Performance','Queue time','Packing time','Total time','Website update'])assert.match(client,new RegExp(label));
