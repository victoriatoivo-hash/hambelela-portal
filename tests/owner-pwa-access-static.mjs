...
assert.match(auth,/refresh_logged_in_user\(\);[\s\S]*role_key[\s\S]*portal_request_is_phone_or_tablet\(\)/);assert.match(auth,/HTTP_SEC_CH_UA_MOBILE/);assert.match(auth,/windows nt\|cros\|x11; linux x86_64/);assert.match(auth,/Desktop Access Required/);assert.match(auth,/workplace desktop/);
