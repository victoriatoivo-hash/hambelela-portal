...
assert.match(css, /\.orders-grid-cell\.orders-grid-cell--select\s*\{[^}]*width:var\(--orders-selection-column-width\)[^}]*height:var\(--orders-table-row-height\)[^}]*justify-content:center/s);
assert.doesNotMatch(css, /--orders-selection-column-width:56px/);
assert.match(css, /\.orders-grid-cell\s*\{[^}]*border-right:1px solid var\(--border\)/s);
