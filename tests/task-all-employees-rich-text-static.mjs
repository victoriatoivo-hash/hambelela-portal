...
const instructionHelpers = fs.readFileSync(new URL('../shared/task-instructions.php', import.meta.url), 'utf8');
...
assert.match(taskPage, /beginTransaction\(\)[\s\S]+foreach \(\$immediateTargetEmployeeIds as \$targetEmployeeId\)[\s\S]+commit\(\)/, 'manual assignment children are created atomically');
...
assert.match(instructionHelpers, /\$allowed = \['p', 'br', 'strong', 'b', 'em', 'i', 'u', 'ul', 'ol', 'li', 'a'\]/, 'server allow-list is explicit');
assert.match(instructionHelpers, /\$blocked = \['script', 'iframe', 'object', 'embed', 'style', 'form', 'input', 'button'/, 'unsafe element families are removed');
