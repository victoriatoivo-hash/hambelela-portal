...
for (const acknowledgement of ['ack_read','ack_principal','ack_instalment','ack_start','ack_deduction','ack_termination','ack_questions']) assert.match(employee, new RegExp(acknowledgement));
...
assert.match(helper, /loan_disbursement/);
assert.match(helper, /loanAgreementPayrollAmounts/);
assert.match(helper, /a\.status='fully_signed'/);
assert.match(helper, /loan_repayment_schedule/);
assert.match(helper, /a\.status='legacy_active'/);
assert.match(payroll, /Employee Loan Payout/);
assert.match(payroll, /loanAgreementSyncPayrollRun/);
assert.doesNotMatch(payroll, /FROM loans WHERE employee_id=\? AND status='active'/);
assert.match(install, /loan_disbursement/);
...
assert.match(helper, /modify\('first day of next month'\)/);
assert.match(helper, /termination/);
assert.match(helper, /EARLY REPAYMENT/);
