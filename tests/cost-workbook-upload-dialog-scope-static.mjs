...
assert.match(page, /<\/main>\s*<dialog id="uploadDialog" class="cw-dialog cw-upload-dialog"/u, 'the namespace class is attached to the live dialog outside the workbook root');
...
assert.match(client, /uploadButton=uploadDialog\?\.querySelector\('#cwUploadSelectedFiles'\)/u, 'JavaScript targets the visible namespaced submit button');
assert.match(client, /uploadButton\.disabled=uploading\|\|selectedUploadFiles\.length===0/u, 'a valid retained PDF or image enables the button while an empty selection disables it');
...
assert.match(client, /if\(uploading\|\|selectedUploadFiles\.length===0\)return/u, 'double submission remains blocked');
