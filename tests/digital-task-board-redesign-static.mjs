...
assert.match(page, /task-trash-table__action-heading">Action/);
assert.match(page, /data-task-action-trigger/);
assert.match(page, /data-task-trash-action="restore"/);
assert.match(page, /data-task-trash-action="delete-permanently"/);
assert.doesNotMatch(page, /packing-trash-action--delete[^\n]*Delete forever/);
assert.match(page, /Task restored successfully\./);
assert.match(page, /Task deleted permanently\./);
assert.match(page, /Only the Owner\/Admin can permanently delete tasks\./);
assert.match(css, /\.task-action-menu__trigger \{[^}]*width:28px[^}]*box-shadow:none !important/);
assert.match(css, /\.task-action-menu \{[^}]*position:fixed[^}]*z-index:60020[^}]*pointer-events:auto/);
assert.match(css, /\.task-action-toast \{[^}]*z-index:60030/);
assert.match(css, /\.task-trash-confirm \{[^}]*z-index:60040/);
...
assert.match(css, /\.digital-task-page \.dtb-page-header \.dtb-page-title \{[^}]*font-weight: 600;/);
