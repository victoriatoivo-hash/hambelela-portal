...
for(const label of ['Orders Still Pending Completion','Completion Compliance','Orders With Unclear Historical Responsibility','Paid and Status Exceptions'])assert.match(service,new RegExp(label));
for(const label of ['Walk-in and Front Desk Order Completion','Clock start','Business duration','Applicable Orders by Fulfilment Mode','Completion Performance by Fulfilment Mode','Orders score explanation','Risk flags'])assert.match(client,new RegExp(label));
