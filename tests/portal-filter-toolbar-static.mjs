...
const packing = readFileSync(new URL('../apps/operations/consignments.php', import.meta.url), 'utf8');
...
assert.match(js, /const portalFilterConfigs = \{/);
assert.match(js, /packing: \{ searchPlaceholder: 'Search packing\.\.\.', fields: \['date', 'status', 'priority', 'person', 'group', 'search'\] \}/);
assert.match(js, /bookkeeping: \{ searchPlaceholder: 'Search bookkeeping\.\.\.', fields: \['date', 'entryType', 'payment', 'person', 'group', 'search'\] \}/);
...
assert.match(presence, /portal-view-bar\.css\?v=shared19/);
assert.match(presence, /portal-view-bar\.js\?v=shared19/);
...
assert.match(ordersJs, /if \(toolbarTrigger === anchor && !toolbarPopover\.hidden\)/);
assert.match(ordersJs, /ordersFilterPanel\.hidden = false/);
assert.match(ordersJs, /ordersFilterPanel\.hidden = true/);
...
assert.match(orders, /<div class="orders-filter-panel" aria-label="Board filters" hidden>/);
assert.doesNotMatch(orders, /class="work-filter-bar portal-filter-panel orders-filter-panel"/);
...
assert.match(js, /classList\.contains\('portal-data-filter-popup'\) \|\| popover\.classList\.contains\('packing-filter-popup'\) \? 560/);
assert.match(js, /portalFilterConfigs\[type\]\?\.searchPlaceholder/);
assert.match(js, /control\.matches\('\[data-packing-group-select\], \[data-bk-filter-group\], \[data-board-group-select\]'\)/);
assert.match(js, /control\.matches\('select'\)\) return control\.selectedIndex > 0/);
assert.match(js, /type === 'packing' \|\| type === 'bookkeeping'/);
assert.match(js, /popover\.classList\.add\('portal-data-filter-popup'\)/);
assert.match(js, /source\.hidden = true/);
assert.match(js, /form\.hidden = false/);
assert.match(js, /movedForm\.hidden = true/);
assert.match(css, /\[data-portal-view-filter\]\[hidden\]\{display:none!important\}/);
assert.match(packing, /data-portal-view-filter aria-label="Packing filters" hidden style="display:none!important"/);
assert.match(bookkeeping, /data-portal-view-filter aria-label="Cash ledger filters" hidden style="display:none!important"/);
assert.match(css, /\.portal-data-filter-popup\{width:min\(560px,calc\(100vw - 24px\)\)!important/);
assert.match(css, /\.portal-data-filter-popup \.portal-view-bar__form,.portal-data-filter-popup \.portal-data-filter-grid/);
assert.match(css, /height:30px!important;min-height:30px!important;max-height:30px!important/);
assert.match(bookkeeping, /data-bk-filter-entry-type/);
assert.match(bookkeeping, /data-bk-filter-payment/);
assert.match(bookkeeping, /data-bk-filter-person/);
assert.match(bookkeeping, /function updateFilteredBookkeepingStats\(\)/);
assert.match(packing, /Search Packing/);
