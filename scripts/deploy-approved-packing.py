...
APPROVED = "f70def1dab5f5d45da01f74cca749809f84aae94"
BASELINE = "e9da16165e8bb0c5593197b8aec27d3df9f998c5"
COMMIT_FILES = (
    "apps/operations/consignments.php",
    "apps/operations/packing-list-action.php",
    "assets/css/packing-essentials.css",
    "assets/js/packing-list.js",
    "tests/packing-manual-distribution-static.mjs",
)
DEPLOY_FILES = COMMIT_FILES[:4]  # Tests stay in GitHub, not the web root.
...
def report(state, **extra):
    with open("packing-deployment-report.json", "w", encoding="utf-8") as handle:
...
def deploy(sha):
    ...
    try:
        ...
        with zipfile.ZipFile("packing-deployment-backup.zip", "w") as archive:
