"""Deploy only the approved Task recurring and Orders identity runtime files."""
...
BASELINE = "6e7ca9144fce1f25ff0a26f4ab85ef73cd5ebce4"
FILES = (
    "apps/operations/checklists.php",
    "apps/operations/partials/checklist-recurring-tasks.php",
    "assets/css/task-recurring-refinement.css",
    "apps/operations/orders-board.php",
    "assets/css/orders-essentials.css",
    "assets/js/orders-board.js",
)
REPORT = "task-orders-refinement-deployment-report.json"
BACKUP = "task-orders-refinement-deployment-backup.zip"
...
def main(mode, approved_sha):
    ...
    if any(data is None for data in expected.values()):
        raise RuntimeError("An approved runtime file is missing from the release commit")
    ...
    try:
        ...
        if conflicts:
            ...
            save_report(report)
            raise RuntimeError("Live Task/Orders files differ from the approved baseline; nothing was uploaded")
...
if __name__ == "__main__":
    if len(sys.argv) != 3 or sys.argv[1] not in ("preflight", "deploy"):
        raise SystemExit("Usage: deploy-task-orders-refinement.py preflight|deploy APPROVED_SHA")
    try:
