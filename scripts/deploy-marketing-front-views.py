...
def report(state,**extra):
    with open("marketing-front-views-report.json","w") as out: json.dump(dict(state=state,sha=APPROVED,base=BASELINE,files=DEPLOY_FILES,**extra),out,indent=2)
def validate(sha):
...
def deploy(sha):
    ...
    try:
        ...
        with zipfile.ZipFile("marketing-front-views-backup.zip","w") as z:
