"""Deploy only the approved Orders performance runtime files with rollback."""
...
REPORT = "orders-performance-deployment-report.json"
BACKUP = "orders-performance-deployment-backup.zip"
...
def main(mode, approved_sha):
    ...
    try:
        ...
        if conflicts:
            ...
            save_report(report)
            raise RuntimeError("Live Orders files differ from the approved baseline; nothing was uploaded")
...
if __name__ == "__main__":
    if len(sys.argv) != 3 or sys.argv[1] not in ("preflight", "deploy"):
        raise SystemExit("Usage: deploy-orders-performance.py preflight|deploy APPROVED_SHA")
    try:
