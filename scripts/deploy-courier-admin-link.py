"""Deploy only the approved Courier runtime and stylesheet with rollback."""
...
BASELINE = "b79330a4"
FILES = (
    "apps/operations/courier.php",
    "assets/css/courier-essentials.css",
)
REPORT = "courier-admin-link-deployment-report.json"
BACKUP = "courier-admin-link-deployment-backup.zip"
...
def main(mode, approved_sha):
    ...
    try:
        ...
        if conflicts:
            ...
            save_report(report)
            raise RuntimeError("Live Courier files differ from the approved baseline; nothing was uploaded")
...
if __name__ == "__main__":
    if len(sys.argv) != 3 or sys.argv[1] not in ("preflight", "deploy"):
        raise SystemExit("Usage: deploy-courier-admin-link.py preflight|deploy APPROVED_SHA")
    try:
